...
> **Folge:** **Lieferung 1 (Autofill stilllegen) ist damit unbegründet und nicht mehr empfohlen** — der Autofill liefert für ~71 Kassen vermutlich die richtige IK (Ops #264). **Lieferung 2 (View + IK-Suche gegen `kostentraeger`) bleibt gültig:** der Podologe tippt die **Karten-IK** (z. B. DAK `100167999`), und die steht nur in `kostentraeger`, nicht in `krankenkassen.ik_number` (= abrechnende IK). Die Migrationsnummer wird `0040` oder höher: `0039` ist belegt (`0039_seed_heilmittel_katalog_podo_komplex_suche.sql`, andere Session, uncommitted), und #301 plant ebenfalls eine Migration — die Nummer wird erst beim Anlegen vergeben, damit sich beide nicht überschneiden. Alle Aussagen unten zu „falsche IK im DTA" und zu `krankenkassen.ik_number` sind Stand 06.09. und nicht mehr aktuell.
