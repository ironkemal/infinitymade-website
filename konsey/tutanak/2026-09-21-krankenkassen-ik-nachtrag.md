...
5. **Migration `api-backend/db/migrations/0040_krankenkassen_ik_nachtrag.sql (0038 = Ops #302, 0039 = Ops #300 View)`:** Datei zuerst, per `id`, nie per Name; Füllen `WHERE ik_number IS NULL`, Leeren `WHERE ik_number = '<mock>'`; Selbstprüfung relativ, nicht „exakt 75 Zeilen" (Kundenbox hat überall NULL, 0009): die 4 geschriebenen IKs existieren in `kostentraeger` mit `datensatz_status='echt'`, keine Mock-IK bleibt übrig. Rollback = neue Datei (SHA-Verriegelung); die 5 Alt-Werte als Kommentar in der Datei. Dump im selben Commit, dann live. Prod-Write nur nach ausdrücklichem OK.
...
- [ ] Migration 0040 schreiben, Dry-Run + Rollback-Plan zeigen — K2
...
- **Entscheidung Melih (21.09.2026): Resolver-Fix lokal committen, kein Push; Prod-Write „noch nicht"** (MCP-Token nicht gesetzt). Migration 0040 bleibt außerhalb des Repos (~/praxura-ops301-backup), nicht im Repo.
- **Nicht getan, bewusst:** `funktionen/INDEX.json` nicht neu erzeugt (ein paralleler Prozess mit Ops #302 arbeitet im selben Arbeitsverzeichnis) — beim Commit `node tools/funktionskarte.mjs`; `erwartete-zaehler.json` (`bis_version` → 0040, `_hinweis_0040` — erst nachdem die 0039 der anderen Sitzung committet ist) nicht angefasst, gleiche Begründung.

### Nachtrag 4 — Migration 0040 im Repo, Prod steht noch aus (21.09.2026, ~22:50)
- **Committet: `fe0b44f`** (lokal, nicht gepusht) — `api-backend/db/migrations/0040_krankenkassen_ik_nachtrag.sql` (SHA-256 `dcbae796…`), `erwartete-zaehler.json` (`bis_version` 0040 + `_hinweis_0040`, Kural 6), `db/REGISTER.md` (Eintrag `krankenkassen` neu, Kundenbox-Notiz), `db/SCHEMA.sql` (Kommentar). Alle sechs Pre-Commit-Gates bestanden. Reihenfolge eingehalten: Ops #300 (`0039`, `a2d8e68`) war zuvor committet.
