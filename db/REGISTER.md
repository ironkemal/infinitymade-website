...
- **Achtung — `no_show_session_links` (Migration `0016`, 14.09.2026, Ops-Karte a8186cb8):** seit dieser Migration gibt auch „Patient nicht erschienen" die verordnete Einheit wieder frei — `prescription_sessions.booking_id = NULL`, `status` zurück auf `'planned'`. Vorher blieb die Zeile am Termin hängen: die Verordnung zeigte 3/6, obwohl nur 2 Einheiten erbracht waren, und die ausgefallene Einheit tauchte nie wieder als offen auf.
  - **Die Spur bleibt am Termin**, nicht an der Sitzungszeile: `status='no_show'` + `no_show` + `no_show_noted_at` + `cancellation_reason`. Die Sitzungszeile ist ein Zähler für erbrachte Leistung, kein Tagebuch (Nutzerentscheidung 14.09.2026).
  - **Wozu die Spalte:** ausschliesslich als Rückfahrkarte für `korrigiereNoShow()` („der Patient war doch da"). Der fand seine Zeile über `.eq('booking_id', …)`; nach der Freigabe findet er dort nichts mehr und die erbrachte Einheit ginge lautlos verloren. **Kein Sitzungszähler, keine Abrechnungsquelle.**
  - **Warum nicht `cancelled_session_links`:** jene Spalte gehört dem #192-Trigger, der sie bei jedem UPDATE auf den alten Wert zurücksetzt — für `no_show` ist sie unbeschreibbar. Der Trigger wurde bewusst **nicht** umgebaut (er hält die Absage-Invarianten zusammen); stattdessen eine additive Spalte, die der Anwendungscode füllt (`module/termin-nicht-erschienen.js`). Damit ist dies die **einzige** der drei Historienspalten an `bookings`, die nicht trigger-eigen ist.
  - **Backfill in derselben Migration:** Alt-no_shows halten ihre Einheiten bis heute fest — erst Momentaufnahme, dann Freigabe, nur für Zeilen in `planned`/`no_show` (eine `done`-Zeile ist abgehakt und womöglich abgerechnet, sie wird nicht angefasst).
  - ⚠️ **Stand 14.09.2026 im SaaS noch nicht angewendet** — wartet auf Gegenlesen. Der Anwendungscode erkennt die fehlende Spalte und fällt dann auf das alte Verhalten zurück (Zeile bleibt am Termin), damit keine Einheit ohne Rückfahrkarte freigegeben wird.
...
- **Achtung — `podologie_altbestand_vor_2023` (boolean) + `podologie_altbestand_beantwortet_am` (timestamptz), seit 17.09.2026 · `0018_leads_podologie_altbestand`, Ops #244:** NULL = noch nicht gefragt. HPNR 78040 hat keinen Anspruch, wenn `true` (Patient vor dem 01.11.2023 erstmals podologisch behandelt, Änderungsvereinbarung 20.10.2023). Patientenbezogen, nicht verordnungsbezogen — deshalb hier und nicht an `prescriptions` (wissensbank/SPEC-RULES.md:139-168). ⚠️ Migration legt nur die Spalten an, der Schreibweg aus der Maske (`module/verordnung-podo.js`, bisher nur die fluechtige `_altbestand`-Variable) steht noch aus — Ops #244 ist damit nicht erledigt, nur entblockt.
- **Achtung — jede Schreiboperation wird protokolliert (seit 17.09.2026, `0021_audit_write_trigger`, Ops #254 / Sicherheitsregister A-18):** Trigger `trg_audit_write_leads` (AFTER INSERT/UPDATE/DELETE) schreibt je geänderter Zeile einen Eintrag nach `data_access_log` (`method='DB'`). Protokolliert werden nur **Spaltennamen**, nie Werte — wer das ändert, baut eine zweite Patientenakte ohne Löschkonzept. Lesezugriffe bleiben bewusst unprotokolliert. Siehe `data_access_log`.
...
- **Achtung — `wagner_grad` wird seit 18.09.2026 nicht mehr aus der Maske geschrieben (Kemal):** das Feld nahm Platz und wurde nicht ausgefüllt. Die Spalte bleibt (CHECK 0–5, von der Festschreibung 0020 bewacht); Altbestand wird in `module/verordnung-detail.js` weiter angezeigt. `podoVerordnungsfelder()` (`module/verordnung-podo.js`) lässt den Schlüssel **weg** statt `null` zu senden — ein `null` löschte beim Speichern jeden vorhandenen Wert und scheiterte bei festgeschriebenen Verordnungen am Trigger. ⚠️ **Damit gibt es fuer Wagner derzeit KEINEN lebenden Erfassungsweg** — die Zeile hier sagte zuerst „liegt in `fußstatus.wagner_grad`“, das ist falsch: `"fußstatus"` ist **veraltet** (kein Code mehr, am 17.09.2026 zum Zählen und Droppen vorgemerkt, Ops #253), und der lebende Fußbefund `pat_fussbefund` hat gar kein Wagner-Feld (`befund` jsonb — `module/fussbefund.js` kennt keines). Die i18n-Schlüssel `fuss_wagner` (de/en/tr, `dashboard.js:251,438,621`) hängen seit der alten Maske ohne Oberfläche in der Luft. Bestand steht ausschließlich in dieser Spalte. Will die Praxis Wagner wieder dokumentieren, ist der Ort `pat_fussbefund.befund`, nicht `prescriptions` und nicht `fußstatus`.
...
- **Achtung — GoBD-Festschreibung (seit 17.09.2026, `0020_prescriptions_festschreibung`, Ops #167):** Trigger `trg_prescriptions_festschreibung` sperrt ab gesetzter `belegnummer` (Tor: `OLD.belegnummer IS NULL` laesst Entwuerfe frei editierbar) den Abrechnungsinhalt aller vier Fachbereiche — vollstaendige Spaltenliste im Migrationskommentar bzw. `db/SCHEMA.sql` bei dieser Tabelle. **Anonymisierungs-Ausnahme:** `patient_name`, `versichertennummer`, `patient_id` duerfen NUR auf NULL gesetzt werden. **Bewusst offen** bleiben `status`, `absetzung_*`, `storno_*`, `zuzahlung_kassiert_*`, `zuzahlung_zahlart`, `bericht_*`, `confirmed_*` — die Folgeprozesse nach der Einreichung. **Kein DELETE-Schutz:** `prescriptions` steht in `api/dsgvo.js` `DELETE_TABLES` (Hard-Delete), ein BEFORE-UPDATE-Trigger sieht ein DELETE nie. ⚠️ Offene Folgefrage (nicht Teil dieser Migration): gesendete Verordnungen werden heute ohne Aufbewahrungsfrist komplett geloescht — an `gkv-302`+`legal-de` geflaggt. Kolonlisten-Entscheidung: db-ustasi + gkv-302 + legal-de, Konsultation 17.09.2026; live getestet (offenes Feld ging durch, gesperrtes Feld warf Fehler, Anonymisierung ging durch).
- **Achtung — jede Schreiboperation wird protokolliert (seit 17.09.2026, `0021_audit_write_trigger`, Ops #254 / Sicherheitsregister A-18):** Trigger `trg_audit_write_prescriptions` (AFTER INSERT/UPDATE/DELETE) schreibt je geänderter Zeile einen Eintrag nach `data_access_log` (`method='DB'`). Protokolliert werden nur **Spaltennamen**, nie Werte — wer das ändert, baut eine zweite Patientenakte ohne Löschkonzept. Lesezugriffe bleiben bewusst unprotokolliert. Siehe `data_access_log`. Kein Konflikt mit der Festschreibung eine Zeile höher: die entscheidet BEFORE, ob die Änderung erlaubt ist, das Audit hält AFTER fest, dass sie passiert ist — blockt die Festschreibung, entsteht korrekterweise kein Eintrag. Weil die Festschreibung `updated_at` setzt, steht die Spalte fast immer in `geaenderte_spalten`.
- **Achtung — `abrechnung_status_manuell_am`/`_von` (seit 18.09.2026, `0025_abrechnung_status_manuell`, Ops #310):** Beta-1 wollte die automatische „bereit zur Abrechnung"-Markierung als reinen Reminder, nicht als bindende Entscheidung — sie sollte sich von Hand zurücksetzen lassen, ohne dass die Automatik sie beim nächsten Durchlauf still wieder hochsetzt. Vorher konnte genau das passieren: `abrechnung_status = NULL` bedeutet sowohl „nie automatisch gesetzt" als auch „von Hand auf 'aktiv' zurückgesetzt" — dieselbe Bedingung, die die Automatik (`.is('abrechnung_status', null)` in `module/podologie-abrechnung.js` UND `module/sitzungsfortschritt.js`, Physio/Ergo/Logo) beim nächsten Durchlauf erneut abfing. Der Stempel wird ausschließlich von der gated Route `PATCH /billing/verordnung/:id/abrechnungsstatus` gesetzt (jeder erfolgreiche manuelle Wechsel, nicht nur nach 'abrechenbar'), und beide Automatik-Stellen prüfen ihn jetzt zusätzlich mit `IS NULL`, bevor sie `abrechnung_status` automatisch auf 'bereit' heben. `abrechnung_status_manuell_von` ist FK auf `auth.users(id)` (nicht `profiles`, Muster wie `booking_status_korrekturen.geaendert_von`) `ON DELETE SET NULL`. Bewusst NICHT in `prescriptions_festschreibung()` (0020) gesperrt. `module/verordnung-topf.js` (`inTopf()`) hat einen zweiten, aktuell toten Schreibpfad auf `abrechnung_status` (kein Aufrufer in Produktion, Stand 18.09.2026) — der stempelt noch nicht; siehe Kommentar dort, falls der Pfad je aktiv wird.
...
- **Achtung — Podologie führt hier bewusst KEINE Zeilen (bestätigt 18.09.2026):** der Terminbereich zeigt für podologische Verordnungen trotzdem „offene Einheiten" mit ihrer fälligen Befundung und lässt sie auf den Kalender ziehen — **berechnet**, nicht gelesen: `module/podo-einheiten.js` (offen = `anzahl_einheiten` − Termine mit `bookings.verordnung_id`, Befund je Einheit aus `sitzungsplan()`). Zwei Prüfungen (`db-ustasi` und `fonksiyon-ustasi`, beide 18.09.2026) haben gezeigt, was ein Hauptbuch für die Podologie kosten würde: ein zweiter Zähler auf `prescriptions.status`/`abrechnung_status` (`module/sitzungsfortschritt.js` gegen `module/podologie-abrechnung.js` — die Verordnung käme nie auf 'bereit' und fehlte lautlos in der §302-Liste), ein zweiter Termin↔Verordnung-Bezug neben `bookings.verordnung_id` (die `codex_192_*`-Trigger leeren `booking_id` bei Absage, `verordnung_id` nie), und **24 Altzeilen** an podologischen Verordnungen (18 planned / 6 done, 12.–19.08.2026, aus Physio-Scans vor der Zusammenlegung), die im Panel plötzlich auftauchten. Eine gespeicherte Zeile trüge zudem keine Positionsnummer: die hängt an der Behandlungsgeschichte des Patienten, nicht an der Sitzung. Vorsorglich gekapselt: `hauptbuchFuehrtStatus()` (`module/sitzungsfortschritt.js`) verweigert der Podologie die Statusschreibung aus diesem Hauptbuch. **Wer das je ändern will, geht zuerst zum Konsey** — die Alternative ist nicht „Zeilen anlegen", sondern die drei Punkte oben zu lösen.
...
- **✅ Lücke geschlossen 17.09.2026 (Ops #167, `0020_prescriptions_festschreibung`):** `prescriptions` hat jetzt einen GoBD-Festschreibungs-Trigger (`prescriptions_festschreibung()` / `trg_prescriptions_festschreibung`, BEFORE UPDATE) — erstmals AUCH fuer Physio/Ergo/Logo, die nie geschuetzt waren. Details, vollstaendige Spaltenliste und die Anonymisierungs-Ausnahme: siehe `prescriptions`-Eintrag unten. Diese Zeile bleibt als historischer Hinweis stehen: die alte Spaltenliste von `trg_verordnungen_festschreibung` war die Vorlage, aus der db-ustasi + gkv-302 + legal-de die neue Liste abgeleitet haben (Konsultation 17.09.2026).
...
- **Seit:** 13.06.2026 · `create_podologie_behandlungen` (Team-SELECT nachgezogen: 03.09.2026 · `verordnungen_podologie_behandlungen_team_select`; `employee_id` nachgezogen: 17.09.2026 · `0019_podologie_behandlungen_employee_id`, Ops #252)
- **Status:** aktiv
...
- **Achtung:** Team darf seit 03.09.2026 LESEN (Policy `Employees can view team podologie_behandlungen`), Schreiben bleibt beim Inhaber. Kein UI dafuer: `podologie-billing` ist in `nav-registry.js` `roles: ['owner']`.
- **Achtung — `employee_id` (uuid, FK `profiles(id)` ON DELETE SET NULL), seit 17.09.2026 · `0019_podologie_behandlungen_employee_id`, Ops #252 (Sicherheitsagent-Fund):** hält fest, wer behandelt hat — Voraussetzung für eine echte Team-Schreib-Policy nach § 630f BGB. NULL bei Altbestand-Zeilen und wenn der Owner selbst behandelt (Owner ist kein employee). ⚠️ Migration legt nur die Spalte an: der einzige INSERT (`module/podologie-abrechnung.js:773`) befuellt sie noch NICHT aus dem eingeloggten Nutzer — bis dahin bleibt jede neue Zeile NULL und das teamweite Schreiben bleibt aus demselben Grund wie vorher gesperrt. Ops #252 ist mit der Spalte nicht erledigt, nur entblockt.
- **Achtung — jede Schreiboperation wird protokolliert (seit 17.09.2026, `0021_audit_write_trigger`, Ops #254 / Sicherheitsregister A-18):** Trigger `trg_audit_write_podologie_behandlungen` (AFTER INSERT/UPDATE/DELETE) schreibt je geänderter Zeile einen Eintrag nach `data_access_log` (`method='DB'`). Protokolliert werden nur **Spaltennamen**, nie Werte — wer das ändert, baut eine zweite Patientenakte ohne Löschkonzept. Lesezugriffe bleiben bewusst unprotokolliert. Siehe `data_access_log`. `business_id` bleibt in diesen Zeilen NULL — die Tabelle hat die Spalte nicht.
- **Achtung — seit 20.09.2026 UNVERÄNDERLICH und NICHT LÖSCHBAR** (`0026_podologie_behandlungen_storno`, Entscheidung **K3** von `legal-de`, live angewandt 20.09.2026). Grundlage: **§ 630f Abs. 1 S. 2 BGB** — eine Berichtigung muss den ursprünglichen Inhalt **erkennbar** lassen, und die Frist beginnt im Moment der **Aufzeichnung**, nicht mit der Abrechnung; dazu **BGH VI ZR 84/19** („Durchstreichen statt Radieren": eine elektronische Dokumentation ohne sichtbare Änderungshistorie hat keinen Beweiswert). `trg_podologie_behandlungen_kein_delete` wirft bei **jedem** DELETE bedingungslos, `trg_podologie_behandlungen_festschreibung` bei jeder Änderung an `behandlungsdatum`, `hpnr_codes`, `diagnosegruppe`, `lokalisation`, `notizen`, `betrag_gkv`, `owner_id` und `created_at`. **Korrektur = Stornierung** (`storniert_am` + `storno_grund` + `storniert_von`, Grund per CHECK Pflicht) und daneben eine **neue** Zeile; die stornierte bleibt lesbar und zählt nicht mehr — weder für die Einmaligkeitssperre noch für die §302-Datei. Eine Stornierung lässt sich **nicht zurücknehmen** (sonst wäre sie eine Notiz, kein Beleg). ⛔ **Kein Kulanzfenster** — auch „innerhalb von 5 Minuten löschbar" wäre nicht gedeckt.
- **Achtung — drei Spalten dürfen trotzdem auf NULL, und das ist Pflicht, nicht Nachlässigkeit.** `verordnung_id`, `employee_id` und `storniert_von` hängen an `ON DELETE SET NULL`, und PostgreSQL führt SET NULL als **UPDATE** aus — der BEFORE-UPDATE-Trigger feuert dabei mit. Wären sie hart gesperrt, ließe sich ab sofort **keine Verordnung, keine Rechnung und kein Mitarbeiterkonto mehr löschen**. Erlaubt ist deshalb genau Wert → NULL, nicht Wert → anderer Wert. `invoice_id` bleibt ganz offen (die Rechnungsbrücke setzt sie).
- **⛔ Folge außerhalb der Datenbank: `podologie_behandlungen` steht seit 20.09.2026 NICHT MEHR in `DELETE_TABLES` von `api/dsgvo.js`.** Bliebe sie drin, würde der BEFORE-DELETE-Trigger werfen und **jede Kontolöschung in einer 500 enden**. In `USER_TABLES` (Auskunft) bleibt sie. Der offene Rest ist ein Auslagerungspaket nach GoBD Rz. 142 ff., gemeinsam mit `belegliste`, `invoices`, `abrechnung` und `patient_consents` — Auslöser: der erste echte Löschantrag.
- **Achtung — die Datumsschranke fehlt weiterhin (geprüft 19.09.2026, unverändert nach 0026):** es gibt keine Prüfung `behandlungsdatum <= heute`, weder CHECK noch Trigger noch Formular. Ein in der Zukunft liegendes Behandlungsdatum lässt sich speichern und kippt danach die **gesamte** §302-Datei dieses Kostenträgers (`S:01005` Leistungsdatum in der Zukunft, `S:01006` nach Rechnungsdatum — `api-backend/billing/dta/preflight.js:408/412`). ⚠️ **Seit 0026 ist das gewichtiger, nicht geringer:** die zwei Testzeilen vom 19.09.2026 ließen sich noch per SQL entfernen — heute ginge das nicht mehr, eine solche Zeile kann nur noch **storniert** werden. Die fehlende Oberfläche dafür steht als Anomalie in `canli-test/REGISTER.md`.
- **Historie (bis 19.09.2026, überholt durch den Absatz oben):** die Tabelle trug außer dem Audit-Trigger **keinen** Schreibschutz — die Festschreibung hängt an `invoices` (`trg_invoice_festschreibung`) und `belegliste` (`trg_prevent_belegliste_mod`), nicht hier. Eine Sitzung ist damit genau so lange löschbar, wie `invoice_id IS NULL` ist und keine `abrechnung_zeile` auf ihre Verordnung zeigt; auf die Tabelle selbst existiert kein einziger eingehender Fremdschlüssel. Umgekehrt fehlt jede Prüfung `behandlungsdatum <= heute` — weder CHECK noch Trigger noch Formular. Ein in der Zukunft liegendes Behandlungsdatum lässt sich speichern und kippt danach die **gesamte** §302-Datei dieses Kostenträgers (`S:01005` Leistungsdatum in der Zukunft, `S:01006` nach Rechnungsdatum — `api-backend/billing/dta/preflight.js:408/412`), ohne dass eine Oberfläche die Sitzung zeigen oder entfernen könnte. Am 19.09.2026 sind so zwei Testzeilen entstanden und mussten per SQL entfernt werden; die fehlende Oberfläche steht als Anomalie in `canli-test/REGISTER.md`.
...
- **Achtung — zehn neue Spalten am 20.09.2026** (§302-Echtbetrieb, Faz 1; `0027_abrechnung_auftragsdatei_und_hashes`, `0029_datenaustausch_zaehler`, `0033_abrechnung_verworfen` — alle drei live angewandt). Drei Gruppen, drei verschiedene Gründe:
  - **`auftragsdatei_path` / `auftragsdatei_size`** (0027): `buildDtaFile()` erzeugte die Auftragsdatei (348-Byte-Auftragssatz, GGT Anlage 2) seit dem 17.09.2026 und **warf sie weg** — keine der drei Erzeugungsrouten nahm sie entgegen. Anhang 2 zur Anlage 1 TP5 Kap. 9 § 3.1 (Prüfstufe 1) verlangt, dass Auftrags- und Nutzdatei **paarweise** übermittelt werden; fehlt die Auftragsdatei, wird abgelehnt, **ohne dass der Inhalt gelesen wird**.
  - **`dta_sha256` / `auftragsdatei_sha256` / `signed_sha256`** (0027, `guvenlik` Ö1): eine Prüfsumme **je Stufe**, weil Signatur (und später Verschlüsselung) die Bytes verändern. Beantwortet die Frage „war das diese Datei?" — der Storage-Pfad allein kann sie nicht beantworten, alle Uploads laufen mit `upsert: true`. Ein SHA-256 ist kein Personenbezug.
  - **`betriebsart`, `datenaustauschreferenz`, `transfernummer`, `empfaenger_ik`** (0027/0029): welche Nummern **diese** Datei getragen hat und an **wen** sie ging. Nicht neu berechnen — sie sind bei der Kasse hinterlegt. `betriebsart` ist eine **Kopie zum Erzeugungszeitpunkt** (CHECK `test|erprobung|echt`, NULL = vor Einführung erzeugt, faktisch `test`): die Einstellung kann sich später ändern, die Datei nicht. `empfaenger_ik` ist Voraussetzung dafür, im Nachhinein überhaupt zu wissen, **welche** Zählerfolge die Datei fortgeschrieben hat — der Zähler läuft je Paar (Absender-IK, Empfänger-IK), siehe `datenaustausch_zaehler`.
- **Achtung — fünf neue Spalten am 22.09.2026: die Verschlüsselung** (`0042_abrechnung_verschluesselung` — ursprünglich als 0039 geschrieben, wegen Kollision mit Melihs parallel gelandeten Migrationen 0039–0041 umnummeriert, live angewandt). `verarbeiteVerschluesselungsSchritt()` erzeugte die CMS-EnvelopedData-Datei und gab sie **nur im HTTP-Response** von `/upload-signed` zurück — nirgends gespeichert. Nach einem Reload wusste niemand mehr, ob eine verschlüsselte Datei überhaupt existiert; der Download-Knopf bot deshalb ausschliesslich die **unverschlüsselte** signierte Datei an, und ein fehlendes Empfängerzertifikat blieb dem Inhaber unsichtbar (O-131, Kaltprüfung 21.09.2026).
  - **Fünf Spalten, kein JSON:** `encrypted_storage_path`, `encrypted_sha256`, `verschluesselt_am`, `verschluesselt_fuer_fingerprint`, `verschluesselung_hinweis` — dasselbe Muster wie die vorhandene `signed_*`-Gruppe, nicht eine vierte Schreibweise desselben Konzepts.
  - **Bewusst KEIN `verschluesselt boolean`.** Das wäre eine zweite Wahrheitsquelle neben `encrypted_storage_path IS NOT NULL` und liefe zwangsläufig auseinander. Der Zustand ist aus den übrigen Spalten ablesbar: Pfad NULL + Hinweis NULL = noch nicht versucht · Pfad NULL + Hinweis gesetzt = versucht und fehlgeschlagen (Grund im Hinweis) · Pfad gesetzt = erfolgreich (Hinweis NULL).
  - **`verschluesselt_fuer_fingerprint`** hält `empfaenger_zertifikate.fingerprint_sha256` des benutzten Empfängerzertifikats. Annahmestellen-Zertifikate rotieren; ohne diese Spalte ist nach einer Rotation nicht mehr beantwortbar, **mit welchem Schlüssel** eine ältere Datei verschlüsselt wurde und ob die Kasse sie noch öffnen kann. Gleicher Zweck wie `signed_by_cert_thumbprint`, nur für den Empfänger statt den Absender.
  - ⚠️ **Schreibregel — die fünf Spalten sind EINE Gruppe und werden bei JEDEM Lauf gemeinsam neu geschrieben.** Erfolg füllt Pfad/SHA/Zeit/Fingerprint und setzt den Hinweis auf NULL; jeder Fehlschlag setzt die vier auf NULL und schreibt den Hinweis. Grund: `/upload-signed` ist wiederholbar (Storage-Upload mit `upsert: true`). Würde nur bei Erfolg geschrieben, bliebe nach einem fehlgeschlagenen zweiten Lauf die verschlüsselte Datei des **ersten** Laufs als scheinbar aktuell stehen, obwohl `encrypted_sha256` dann nicht mehr zum neuen `signed_sha256` gehört. Wer hier eine „nur bei Erfolg schreiben"-Abkürzung einbaut, erzeugt genau diese stille Fehlpaarung.
  - `verschluesselung_hinweis` ist **PHI-frei** zu halten (Ö5): strukturelle Meldungen wie „kein Verschlüsselungszertifikat vorhanden", nie Rohdateiinhalt und keine Patientendaten. Pfad, SHA-256 und Fingerprint sind kein Personenbezug — deshalb auch hier **kein** Eintrag in `api/dsgvo.js`, und keine neue Policy nötig: die Spalten erben die Policy der Tabelle.
  - **Lesende Seite:** `module/abrechnung-detail.js`, `module/abrechnung-verlauf.js` und der DAS-Guide-Knopf in `dashboard.js` bevorzugen seit dem 22.09.2026 `encrypted_storage_path` und geben `signed_storage_path` nicht mehr stillschweigend heraus. Wer eine neue Download-Stelle baut, hält sich an diese Reihenfolge — sonst wandert wieder eine unverschlüsselte §302-Datei über den Bildschirm.
- **⚠️ Richtigstellung zu 0034: `abrechnung` ist NICHT festgeschrieben.** `0034_abrechnung_uebermittlung.sql` begründet die eigene Transporttabelle unter anderem damit, ein GoBD-Festschreibungs-Trigger schütze `abrechnung` vor nachträglichen UPDATEs. Auf DB-Ebene stimmt das nicht — nachgezählt am 22.09.2026 trägt die Tabelle genau **zwei** Trigger: `abrechnung_updated_at` und `trg_set_business_id`. Die `signed_*`-Gruppe wird schon heute nachträglich per UPDATE befüllt, die neue `encrypted_*`-Gruppe tut dasselbe. Kein Handlungsbedarf — die eigene Tabelle bleibt aus dem **1:n**-Grund richtig —, aber ein Vermerk für die nächste Person, die aus jenem Satz schliesst, eine neue Spalte an `abrechnung` brauche zwingend eine eigene Tabelle. (Festgeschrieben sind in dieser Familie `abrechnung_zeile`, `abrechnung_zahlung`, `abrechnung_uebermittlung` und `belegliste` — **nicht** der Kopfsatz.)
- **Achtung — `status` kennt seit 20.09.2026 den Wert `verworfen`, dazu `verwerfungsgrund`** (`0033_abrechnung_verworfen`). Hintergrund: `vergebeNummern()` zieht Datenaustauschreferenz und Transfernummer **vor** dem Bauen der Datei; schlägt danach `buildDtaFile()` fehl (z. B. Preflight), sind die Nummern verbraucht und es entsteht eine Lücke. **Die Reihenfolge bleibt absichtlich so** — umgedreht würden die Regeln F:03001–F:03004 geblendet, die genau diese Nummer prüfen. Die Lücke selbst ist **kein** Spezifikationsverstoß (verlangt ist „fortlaufend", nicht „lückenlos"; keine der drei Prüfstufen sucht nach Lücken), aber **GoBD verlangt Erklärbarkeit** — dieselbe Begründung steht bei `nummernkreise`. `verwerfungsgrund` nimmt den Fehlercode bzw. eine gekürzte, **PHI-freie** Zusammenfassung auf; wer dort einen Rohfehler mit Patientendaten hineinschreibt, macht aus einem Betriebsprotokoll eine Akte.
...
- **Achtung — der Weg zur Nummer führt seit 11.09.2026 nur noch über den Trigger.** `naechste_nummer(uuid, text, integer)` und `naechste_verordnungsnummer(uuid, uuid)` haben kein EXECUTE mehr für `anon`/`authenticated`/`PUBLIC` (`api-backend/db/migrations/0002_nummernkreis_rpc_revoke.sql`, Fund S-24). Beide nahmen den Mandanten als Argument entgegen und prüften ihn nicht gegen `auth.uid()` — ein `POST /rest/v1/rpc/naechste_nummer` mit fremder `owner_id` verbrauchte eine Nummer einer fremden Praxis und riss deren lückenlose Nummerierung (§ 146 Abs. 4 AO) von außen auf. **Die Vergabe selbst ist unberührt:** die damals einzigen zwei Aufrufer, `set_invoice_nummer()` und `vergebe_verordnungsnummer_rx()`, sind SECURITY DEFINER und laufen als Funktionsbesitzer (live `postgres`, in der lokalen Box `supabase_admin`), nicht unter der Aufruferrolle. Am 11.09.2026 in der Box mit echten INSERTs nachgewiesen statt nur angenommen: `INV-2026-0001/0002/0003`, eigener Kreis ab `issued_at` 2027, Verordnungsnummern 1/2, UPDATE-Einfrierung hält, Direktaufruf als `anon` und `authenticated` scheitert mit `permission denied for function`. **Ein künftiger Aufrufer muss ebenfalls SECURITY DEFINER sein** — ein SECURITY-INVOKER-Aufrufer bekommt seit 0002 `permission denied`.
- **Achtung — `jahr = 0` heißt „kein Jahresbezug“.** Nur `rechnung` zählt jahresweise (`jahr` = Ausstellungsjahr, Reset am 1.1.). `beleg`, `mahnung` und `ausfallrechnung` liefen nie jahresweise, sondern fortlaufend je Inhaber — sie tragen deshalb den festen Dummy-Wert `0`. Das ist kein Platzhalter für „noch nicht gesetzt“: `nummernkreise.jahr` ist `NOT NULL` und Teil des Primary Key `(owner_id, kreis, jahr)`, ein `NULL` würde bei `ON CONFLICT` nie matchen und den Zähler bei jedem Aufruf auf 1 zurücksetzen. Wer einen neuen Kreis anlegt, entscheidet also bewusst: Jahreszahl **oder** `0`, nichts dazwischen.
- **Achtung — die drei `MAX+1`-Trigger sind seit 11.09.2026 abgelöst** (`api-backend/db/migrations/0003_nummernkreis_beleg_mahnung_ausfallrechnung.sql`). Bis dahin galt: die Zeile „Warum“ von 16.08.2026 nannte `beleg_nr` und `mahnung_nr` mit, eingelöst war nur `rechnung`. `set_next_beleg_nr()`, `set_next_mahnung_nr()` und — beim Nachzählen gefunden, ursprünglich nicht auf der Liste — `set_next_ausfallrechnung_nr()` zählten weiter mit `SELECT COALESCE(MAX(…),0)+1` je `owner_id`, also mit genau dem Rennen, das diese Tabelle abschaffen sollte. Alle drei rufen jetzt `naechste_nummer(NEW.owner_id, '<kreis>', 0)` und sind **SECURITY DEFINER** — zwingend, seit 0002 hat `authenticated` kein EXECUTE mehr auf `naechste_nummer` (siehe Absatz darüber). Der Bestand wurde in derselben Migration nachgezogen (`GREATEST`-Upsert, auf einer leeren Kundenbox ein No-Op); live nachgemessen, `last_nr` traf bei allen sechs Inhaber-Zeilen exakt das vorhandene Maximum.
- **Achtung — der Ernstfall war bei `ausfallrechnung` größer als bei den anderen zweien.** `belegliste` und `mahnungen` haben ein `UNIQUE (owner_id, …_nr)`; ein Rennen endete dort in einem Fehler — ärgerlich, aber sichtbar. `ausfallrechnungen` hat **keinen** solchen Index (nur `PRIMARY KEY (id)`, am 11.09.2026 geprüft) — dort wären zwei Rechnungen mit **derselben** Nummer stillschweigend durchgelaufen, und das ist eine §-14-Abs.-4-Nr.-4-UStG-Frage, keine Komfortfrage. Der Bestand ist sauber (4 Zeilen, 4 verschiedene Paare, keine `NULL`). Ein nachgeschobenes `UNIQUE (owner_id, rechnung_nr)` als Netz ist offen und wäre heute konfliktfrei anzulegen.
- **Achtung — eine Lücke in der Nummernfolge ist ab jetzt möglich und kein Fehler.** `nummernkreise` verbraucht die Nummer beim Ziehen, auch wenn die Transaktion danach zurückrollt; `MAX+1` tat das nicht. GoBD verbietet Lücken nicht, verlangt nur, dass sie erklärbar sind — dieser Absatz ist die Erklärung. `invoices` verhält sich seit 16.08.2026 bereits so; 0003 zieht die anderen drei auf dieselbe Konsistenz. Eine fehlende `beleg_nr` ist also **kein** Hinweis auf einen gelöschten Beleg (Löschen ist dort ohnehin per Trigger gesperrt).
- **Achtung — die `WHEN`-Klausel der Trigger lässt eine Tür offen.** `trg_set_beleg_nr` und `trg_set_mahnung_nr` feuern nur `WHEN (new.…_nr IS NULL OR new.…_nr = 0)`. Ein INSERT mit **explizit gesetzter** Nummer umgeht damit den Trigger **und** den Zähler — `nummernkreise` wüsste nichts davon und die nächste automatische Nummer liefe in den UNIQUE-Index. Heute tut das kein Codepfad (am 11.09.2026 geprüft: die einzige explizite `beleg_nr` steht in `api-backend/billing/api/abrechnung.routes.js:2141` und ist ein Mock für den Fall „Tabelle existiert nicht“, geht nie in die DB). `trg_set_ausfallrechnung_nr` hat diese Klausel **nicht** und ist deshalb dicht. Wer eine Nummer im Code mitgibt, muss `nummernkreise` von Hand nachziehen — besser: gibt sie nicht mit.
...
- **Seit:** 20.09.2026 · `api-backend/db/migrations/0029_datenaustausch_zaehler.sql` (ABRECHNUNG_ECHTBETRIEB_PLAN.md, Schritt 1.2)
- **Status:** aktiv — ✅ **am 20.09.2026 live angewandt** (MCP, zusammen mit 0026–0034). Die Tabelle ist noch leer: die erste Zeile entsteht, sobald die erste Datei unter dem neuen Verfahren erzeugt wird. Ein rückwirkender Backfill aus dem Bestand wurde bewusst **nicht** gemacht — die Altzeilen tragen keinen `empfaenger_ik`, eine je Paar aufgebaute Folge wäre geraten, nicht gewusst. Wer den Zähler vor dem ersten Lauf setzen will, nimmt `datenaustausch_zaehler_vorstellen()`.
- **Wer:** **niemand direkt — mit Absicht, genau wie `nummernkreise`.** Nur die drei SECURITY-DEFINER-Funktionen `naechste_datenaustauschreferenz(owner, absender_ik, empfaenger_ik)`, `naechste_transfernummer(…)` und `datenaustausch_zaehler_vorstellen(…)`. Einziger Aufrufer im Code: `vergebeNummern()` in `api-backend/billing/api/abrechnung.routes.js`, verwendet von allen drei Erzeugungswegen (create, create-podologie, korrektur).
- **Achtung — RLS ist an, es gibt bewusst KEINE Policy.** Gleiche Bauart und gleicher Grund wie `nummernkreise`: kein Client fasst den Zähler an. Der Supabase-Advisor meldet die Tabelle deshalb dauerhaft unter `rls_enabled_no_policy` — das ist **kein Befund**, sondern der Zweck, genau wie bei `nummernkreise`, `kiosk_pins` und `praxura_migrations`.
- **⛔ Achtung — die Falle aus S-24 war hier NICHT von Anfang an zu. Sie stand drei Tage offen** (gefunden und geschlossen am 20.09.2026, `0035_datenaustausch_zaehler_rpc_revoke`). Hier stand bis dahin, der `REVOKE … FROM PUBLIC` in 0029 genüge. Er genügte nicht: `anon` und `authenticated` hatten EXECUTE auf alle drei Funktionen **nicht über PUBLIC**, sondern als **eigenen Grant aus `ALTER DEFAULT PRIVILEGES`** — Supabase vergibt den auf jede neu angelegte Funktion in `public`. Live gemessen (`pg_proc.proacl`) vorher: `{postgres=X, anon=X, authenticated=X, service_role=X}`, nachher `{postgres=X, service_role=X}` (identisch zu `naechste_nummer`). Damit war genau der S-24-Weg offen: der Mandant kommt als **Argument** (`p_owner`, `p_absender_ik`) und wird nicht gegen `auth.uid()` geprüft, SECURITY DEFINER hängt RLS ohnehin aus — ein Fremder hätte die Nummernfolge einer anderen Praxis weiterdrehen oder mit `_vorstellen()` auf einen beliebigen Wert ziehen können, und über `buildSammelRechnungsnummer()` hängt daran auch die **Rechnungsnummer**.
- **Achtung — die Lehre daraus korrigiert die Lehre aus 0024.** 0024 (17.09.2026) hatte festgehalten: „der eigentliche Befund war nicht der anon-GRANT, sondern PUBLIC". Das gilt für **Bestands**funktionen. Für **neu angelegte** Funktionen gilt das Gegenteil — dort kommen die Rechte als explizite Rollen-Grants, und `FROM PUBLIC` nimmt einem einzelnen Grantee nichts weg. **Beide Wege existieren nebeneinander; wer nur einen schließt, schließt nichts.** Deshalb verlangt `tools/check-security-definer-grants.mjs` seit dem 20.09.2026 alle drei (`PUBLIC`, `anon`, `authenticated`) statt nur `PUBLIC`. ⚠️ Das Tor hat **nicht** wegen einer Lücke in der Namensliste versagt — alle drei Funktionen standen seit 0029 darauf; das **Prüfmuster** war zu schwach. Gegenprobe nach der Korrektur: gegen 0029 schlägt es rot aus, gegen 0001 und 0002 bleibt es grün.
- **Achtung — `letzte_referenz` ist `bigint` und läuft NICHT über.** Das UNB-Feld ist fünfstellig; der Fünfsteller entsteht erst bei der Ausgabe (`((n-1) % 99999) + 1`). So überläuft das Spezifikationsfeld, die Historie aber nicht. `letzte_transfernummer` läuft dagegen wirklich im Kreis, **0..999**.
- **Achtung — die vergebenen Nummern stehen zusätzlich auf der Zeile** (`abrechnung.datenaustauschreferenz`, `.transfernummer`, `.empfaenger_ik`). Ohne sie ließe sich eine Datei nicht identisch neu erzeugen, und ein zweiter Sendeversuch bekäme eine andere Transfernummer — die Spezifikation verlangt dieselbe (Anhang 1 § 4.3). Nicht neu berechnen: die Nummer ist bei der Kasse hinterlegt.
- **Achtung — Migration und `restore.sh` drehen den Zähler zurück** (`onprem` O-115). Die `abrechnung`-Tabelle einer frischen Kundenbox ist leer, der Zähler begänne bei 1 und vergäbe Referenzen ein zweites Mal — und aus derselben Nummer entsteht über `buildSammelRechnungsnummer()` auch die **Rechnungsnummer**, es beißt also zusätzlich die GoBD-Seite. Dagegen gibt es `datenaustausch_zaehler_vorstellen()`: **ausschließlich vorwärts** (`greatest(…)`), nie zurück. Ein Umzug oder ein Rücksicherungslauf muss diesen Schritt enthalten. ⛔ Eine Nummernvergabe aus der Zentrale ist ausgeschlossen — das wäre eine neue Typ-A-Abhängigkeit, und eine Box ohne Netz könnte keine Rechnung mehr schreiben.
- **Achtung — `owner_id` ist NICHT Teil des Schlüssels und hängt deshalb an `ON DELETE SET NULL`, nicht an `CASCADE`** (`onprem`, 20.09.2026, Bedingung vor der Anwendung). Der Schlüssel ist `(absender_ik, empfaenger_ik)`. Mit `CASCADE` hätte das Löschen eines Profils die **Zählerzeile** mitgenommen — der Zähler begänne wieder bei 1, und die einzige Zusage, für die diese Tabelle existiert („geht nie zurück"), wäre gebrochen. Die Spalte ist ein Herkunftsvermerk. Eine verwaiste Zeile (`owner_id IS NULL`) heilt sich beim nächsten Ziehen selbst: alle drei Funktionen setzen im `ON CONFLICT`-Zweig `owner_id = coalesce(bestehend, neu)`.
- ~~**Offene Fachfrage an `gkv-302`** zur Transfernummer „0..999" vs. „1..999"~~ — **beantwortet 20.09.2026.** Der Wertebereich ist **0..999**; die Quelle ist **GGT Anlage 2, Feld `TRANSFER_NUMMER`** (Auftragssatz V1.0, Stand 10.10.2024): *„Sie wird ab '999' wieder auf '0' gesetzt."* Anhang 1 § 4.3 nennt nur die Position, **keinen** Wertebereich — das „1..999" im Code war unbelegt. `naechste_transfernummer()` rechnet seitdem `(n + 1) % 1000`; erster vergebener Wert bleibt 1.
...
- **Seit:** 20.09.2026 · api-backend/db/migrations/0031_betriebsart_je_empfaenger.sql (ABRECHNUNG_ECHTBETRIEB_PLAN.md, Schritt 1.7)
- **Status:** aktiv — ✅ **am 20.09.2026 live angewandt** (MCP). Die Tabelle ist leer, und das ist der Normalzustand: eine Zeile entsteht erst, wenn für eine bestimmte Datenannahmestelle vom Vorgabewert abgewichen wird.
- **Wer:** `ladeBetriebsart()` in `api-backend/billing/api/betriebsart.js` liest — aufgerufen aus allen drei Erzeugungswegen in `abrechnung.routes.js` (create, create-podologie, korrektur); `module/abrechnung-einstellungen.js` schreibt (nur Inhaber).
- **Achtung — ohne Zeile gilt der Vorgabewert, nicht `test`.** Die Auflösungsreihenfolge ist: Zeile hier → `terapeut_zertifikat.betriebsart` → `'test'`. Fehlt die Tabelle (Migration noch nicht eingespielt), fällt Backend **und** Oberfläche lautlos auf den Vorgabewert zurück; jeder **andere** Datenbankfehler bricht die Erzeugung ab (Status 500) statt still auf `test` zu fallen — eine still nach `test` gefallene Echtabrechnung erzeugt eine Datei, die keine Zahlung auslöst, und **es kommt keine Fehlermeldung**.
- **Achtung — Offene Fachfrage:** Ob die Krankenkasse die Zulassung je **Kassenart** oder je **einzelner Kasse** erteilt, steht in keiner Quelle und wird im DAS-Termin (Plan Abschnitt 2.1) erfragt.
...
- **Teamzugriff (seit 17.09.2026, Migration `0023_aerzte_ausfall_team_insert`, Ops #299):** Angestellte dürfen **lesen und anlegen** — **den Status nicht ändern**. Die Trennung ist der Kern der Entscheidung: Anlegen gehört an die Anmeldung, wo der Ausfall entsteht und erfasst wird (`handleDirectAusfallrechnung()` → `POST /ausfall/create`, bis dahin über den Service-Role-Client an RLS vorbei — gefunden von S-30); Ermessen hat der Erfassende dabei nicht, Höhe und Hinweistext kommen aus `profiles.ausfall_*` des Inhabers und `pruefeAusfallFrist()` entscheidet über die Fristwahrung. **UPDATE bleibt owner-only**, weil UPDATE hier der Statuswechsel `offen → bezahlt | storniert | abgeschrieben` ist — also eine Geldaussage: „bezahlt“ erklärt eine Forderung für erledigt, „abgeschrieben“ verzichtet darauf. Seit demselben Tag sagt der Code dasselbe: `PATCH /ausfall/:id/status` gibt bei `profile.role !== 'owner'` 403 zurück (Commit 9ac1978). DELETE gibt es weiterhin für niemanden — eine vergebene Rechnungsnummer verschwindet nicht.
...
- **Achtung — trägt seit 20.09.2026 auch die §302-BETRIEBSART** (`0028_terapeut_zertifikat_betriebsart`, live angewandt; ABRECHNUNG_ECHTBETRIEB_PLAN.md Schritt 1.7). Fünf Spalten: `betriebsart` (NOT NULL, Vorgabe `'test'`, CHECK `test|erprobung|echt`), `betriebsart_geaendert_am/_von`, `zulassung_referenz`, `zulassung_datum`. Der Weg in den Echtbetrieb führt über **drei** Stufen, nicht zwei: Testverfahren → UNB-Testindikator **0**, Dateiname `TSOL0nnn` · Erprobung → **1**, Dateiname **weiterhin** `TSOL0nnn` · Echtbetrieb → **2**, `ESOL0nnn`. ⚠️ **In der Erprobung bleibt das „T" im Dateinamen**, obwohl echte Daten rausgehen — das ist gegenintuitiv und jemand wird es „korrigieren" wollen; `filename.js:73` und `auftragsdatei.js:142` machen es bereits richtig.
- **Achtung — der zweite CHECK ist der eigentliche Riegel.** `betriebsart = 'echt'` ist ohne `zulassung_referenz` **und** `zulassung_datum` gar nicht speicherbar. Die Prüfung steht bewusst in der **Datenbank** und nicht (nur) in der Route: `abrechnung` wird auch vom Frontend über PostgREST geschrieben, und eine Regel, die nur in einer von drei Routen steht, ist keine Regel. Die Zulassung erteilt die **Krankenkasse**, nicht die Datenannahmestelle.
- **⛔ Achtung — KEINE Umgebungsvariable, und das ist eine Entscheidung** (`onprem` O-117, 20.09.2026). Im SaaS wäre eine Umgebungsvariable mandantenübergreifend und würde **alle** Praxen gleichzeitig umstellen, auch die ohne Zulassung. In der Kundenbox hieße sie: `.env` bearbeiten und den Container neu erzeugen — der Kunde tut das nicht, und wir kommen nicht hinein (K10). Der Schalter muss dort liegen, wo der Inhaber drankommt. `praxura_setup` schied aus, weil es die Tabelle nur in der Box gibt (zwei Codewege, G7).
- **Achtung — dies ist der VORGABEWERT der Praxis, nicht das letzte Wort.** Die Ausnahme je Datenannahmestelle steht in `betriebsart_empfaenger` (0031). Auflösungsreihenfolge: Zeile dort → `terapeut_zertifikat.betriebsart` → `'test'`.
...
- **`krankenkassen.ik_number` steht in `0009` in jeder Zeile absichtlich auf `NULL`** (Begründung
...
- **Ladestand (20.09.2026 live nachgezählt, nicht fortgeschrieben):** **11.407 Zeilen aus 6 von 6 geladenen Dateien** — `AO05Q326_KE3` 764 · `BK05Q326_KE1` 3.234 · `IK05Q326_KE1` 475 · `BN050526_KE0` 6.183 · `LK05Q226_KE0` 27 · **`EK05Q226_KE0` 724**. ⚠️ Gegenüber dem 07.09.-Stand haben sich zwei Dinge geändert: die Ersatzkassen-Datei ist jetzt die **heute gültige Q2**-Ausgabe statt der zu früh geladenen Q4 (siehe ZEITFEHLER unten — damit ist er **erledigt**), und die Gesamtzahl geht deshalb von 11.409 auf 11.407. `quelle_stand` ist in allen 11.407 Zeilen gefüllt (live geprüft: 0× NULL). Die Summe stimmt Datei für Datei mit den VKG-Zahlen in `wissensbank/REGISTER.md` Kart W-01 überein. Verknüpfungsarten: 969× 01 (Verweis), 211× 02, 2.465× 03 (Datenannahmestelle), 7.764× 09 (Papier). Damit ist der am 06.09. notierte „offene Rest" (4 von 6 Dateien, 2.800 Segmente) **erledigt** — der alte Text stand hier bis zum 07.09.2026 und war falsch.
...
- **~~⛔ ZEITFEHLER~~ — ✅ ERLEDIGT am 20.09.2026.** In der Tabelle steht jetzt `EK05Q226_KE0` (gültig ab 01.04.2026, die heute gültige Ausgabe); `EK05Q426_KE0` ist draußen. Live gegengezählt: 724 Zeilen aus dieser Datei, `quelle` und `quelle_stand` gesetzt. Zusätzlich warnt `tools/kostentraeger-annahmestellen-laden.mjs` jetzt von selbst, wenn ein „gültig ab" in der Zukunft liegt oder zwei Ausgaben derselben Kassenart eingetragen sind — der Lauf vom 20.09.2026 meldete „Stichtagsprüfung: in Ordnung". **Der Befund bleibt unten stehen, weil die Denkfigur wiederkommt, nicht weil der Zustand noch gilt:** ⛔ *geladen war die ZUKUNFT, nicht das Heute.* Für die Ersatzkassen (vdek: TK, BARMER, DAK, KKH, hkk, HEK …) steht `EK05Q426_KE0` in der Tabelle — das ist die Ausgabe **gültig ab 01.10.2026**. Die heute gültige `EK05Q226_KE0` (gültig ab 01.04.2026) wurde **nicht** geladen. Das widerspricht direkt der Regel in `wissensbank/REGISTER.md` W-01: *„7. dosya bugün koda/DB'ye girmez … 01.10.2026'dan önce Q4'ü yüklemek yanlış Datenannahmestelle'ye yönlendirme demektir — §302'de bu, dosya reddi sınıfıdır."* Der Kommentar im Ladescript begründete es mit „Q4 ist der Nachfolger von Q2" — das ist die Denkfigur *„das Neueste ist immer richtig"*, und sie ist bei quartalsweise datierten Stammdaten falsch.
...
- **Achtung:** `EK05Q226_KE0.txt` ist **nicht** die siebte fehlende Datei, sondern die heute gültige Ausgabe derselben Kassenart. ⚠️ Hier stand bis zum 20.09.2026, sie sei „bewusst-falsch ausgelassen" — das gilt nicht mehr: seit dem 20.09.2026 ist genau **sie** geladen und `EK05Q426_KE0` ist es nicht mehr.
- **Achtung — `quelle_stand` fehlte bis 20.09.2026 im Seed-Generator.** `tools/seed-generieren.mjs` führte für diese Tabelle `quelle` in `columns`, aber nicht `quelle_stand` (beim Nachbarn `kostentraeger` stand es drin). Folge: das ausgelieferte `0007_seed_kostentraeger_annahmestellen.sql` schrieb in jeder Kundenbox `quelle_stand = NULL` — und dort ließ sich genau die Frage nicht mehr beantworten, um die es im Zeitfehler oben geht: **wie alt ist diese Empfängerliste?** Die Spalte ist ergänzt, und die Korrektur für die Box liegt seit dem 20.09.2026 als `api-backend/db/migrations/0037_seed_kostentraeger_annahmestellen_quelle_stand.sql` vor: ein vollständiger Re-Seed derselben 11.407 Zeilen, jetzt **mit** `quelle_stand`; das `ON CONFLICT DO UPDATE` trägt den Wert auf bestehenden Box-Zeilen nach. ⛔ `0007` selbst wird **nicht** geändert — der Runner hält je angewandter Datei eine SHA-256, eine nachträgliche Änderung lässt die Box gar nicht mehr starten. ⚠️ Im SaaS ist nichts zu tun und die Datei wird dort auch **nicht** angewendet (wie schon 0006–0015): `quelle_stand` ist hier über das Ladescript längst gefüllt, live geprüft 0 von 11.407 Zeilen NULL.
...
- **Warum:** Die Urbelege (Verordnungen im Original) gehen **per Post** an die **Papierannahmestelle** (Verknüpfungsart **09**), nicht an die Datenannahmestelle (02/03) — Richtlinien-Text § 2(1) und § 4. Der Begleitzettel trug bis zum 20.09.2026 **keine Empfängeradresse**, weil es in der Datenbank keinen Ort dafür gab: `kostentraeger` hat nur IK und Name. Im Echtbetrieb muss der Podologe wissen, wohin der Umschlag geht.
  **Warum eine eigene Tabelle und keine Spalten an `kostentraeger`** (`db-ustasi`, 20.09.2026): das `ANS`-Segment ist wie `VKG` **wiederholbar** — Hausanschrift, Postfach und Großkundenanschrift stehen nebeneinander. Flache Spalten hätten exakt den Fehler von `kostentraeger.das_ik` wiederholt, also eine Einzelspalte für eine 1:n-Beziehung; genau die gilt heute als veraltet. Eine statische Codedatei schied aus demselben Grund aus wie bei den Annahmestellen: die Daten kommen quartalsweise neu.
- **Seit:** 20.09.2026 · `api-backend/db/migrations/0032_kostentraeger_anschriften.sql` (ABRECHNUNG_ECHTBETRIEB_PLAN.md, Schritt 1.4)
- **Status:** aktiv (Referenz) — ✅ **am 20.09.2026 live angewandt** (MCP). Befüllt wird sie von `tools/kostentraeger-annahmestellen-laden.mjs --write` (derselbe Lauf wie die VKG-Zeilen); die Seed-Migration für die Kundenbox entsteht daraus über `tools/seed-generieren.mjs`.
- **Ladestand (20.09.2026 live nachgezählt, nicht fortgeschrieben):** **1.588 Zeilen zu 1.043 verschiedenen IK.** Verteilung nach `art`: **999× '1'** (Hausanschrift) · **489× '2'** (Postfach) · **100× '3'** (Großkunde). Aus dem Rohtext der sechs Dateien geparst wurden **1.630** Adressen; die Differenz von **42** sind im natürlichen Schlüssel exakte Dubletten (dieselbe Anschrift mehrfach genannt) und fallen am UNIQUE weg — nachgerechnet gegen den Parser, nicht geschätzt. `quelle_stand` ist in **allen** 1.588 Zeilen gefüllt.
- **★ Die Zahl, auf die es fachlich ankommt: ALLE 135 Papier-IK haben eine Anschrift — 135 von 135.** Gezählt als `count(distinct partner_ik)` über `kostentraeger_annahmestellen` mit `verknuepfungsart = '09'`, abgeglichen gegen `kostentraeger_anschriften`. Damit trägt der Begleitzettel für jede heute auflösbare Papierannahmestelle eine echte Empfängeradresse; der Klartext-Ersatzhinweis greift für keine davon.
- **Seed für die Box:** `api-backend/db/migrations/0036_seed_kostentraeger_anschriften.sql` (1.588 Zeilen). ⚠️ Wie schon 0006–0015 wird die Datei **nicht** auf die SaaS-Datenbank angewendet — dort sind die Daten über das Ladescript ohnehin vorhanden (G7); die Seed-Dateien existieren für die Kundenbox. Der Rumpf ist nachweislich byte-identisch mit der Ausgabe von `tools/seed-generieren.mjs` (md5-Vergleich gegen dieselbe `format('%L')`-Abfrage in der Live-DB, Begründung im Kopf der Datei — `DATABASE_URL` ist im SaaS bewusst nicht gesetzt, O-91, das Werkzeug erreicht diese Datenbank von dort gar nicht).
- **Wer:** `ladePapierannahmestelle()` in `api-backend/billing/kostentraeger/annahmestelle.js` liest — von dort über `baueBegleitzettel()` in `abrechnung.routes.js` auf den Begleitzettel. Geschrieben ausschließlich vom Ladescript.
- **Quelle:** `ANS`-Segmente derselben Kostenträgerdateien wie `kostentraeger` und `kostentraeger_annahmestellen`. Schlüsselbedeutung: Anhang 03 zu Anlage 1 TP5 § 7 — `art` **'1'** Hausanschrift · **'2'** Postfach · **'3'** Großkundenanschrift.
- **Achtung — die Auswahl passiert beim LESEN, nicht beim Import.** Alle ANS-Zeilen werden unverändert gespeichert; `waehlePostanschrift()` (`billing/kostentraeger/parser.js`) wählt daraus nach der Vorzugsreihenfolge 1 > 2 > 3. Die Priorisierung wird bewusst **nicht** in die Daten eingebrannt — sonst wäre die Information weg, sobald sich die Vorzugsregel ändert.
- **Achtung — die Adresse gehört zur IK der PAPIERANNAHMESTELLE**, nicht zu der des Kostenträgers, dessen VKG-Zeile sie gefunden hat. Das ist der ganze Zweck dieser Tabelle; wer auf `kostentraegerIk` statt auf `treffer.partnerIk` abfragt, schickt die Urbelege an die Kasse statt an die Annahmestelle.
...
### `kostentraeger_auswahl` (Sicht)
- **Warum:** Beantwortet „welche IK darf im Kassenfeld als Krankenkasse angeboten werden?" — für die IK-Suche, die der Podologe mit der **Karten-IK** von Muster 13 macht (Ops #300). `kostentraeger` ist dafür nicht direkt brauchbar: neben den Kassen stehen dort **Rechenzentren und Abrechnungsstellen** („gkv informatik", BITMARCK, Rezeptprüfstelle Duderstadt …), und `payer_type` trennt sie **nicht** (im Seed sind alle 1.043 Zeilen `gkv`). **Warum eine Sicht und keine Tabelle/Spalte:** reine Auswahl über zwei bestehende Tabellen, keine neue Wahrheit; die Bedingung „hat mindestens ein VKG" (`abrechnender_kt_ik IS NOT NULL OR EXISTS annahmestellen`) lässt sich per PostgREST nicht als Filter ausdrücken. Eine RPC hätte die SECURITY-DEFINER-Rechtezeremonie (0035) und den `fonksiyon`-Zähler gekostet; die Sicht kostet keinen Zähler.
- **Seit:** 21.09.2026 · `api-backend/db/migrations/0040_kostentraeger_auswahl_view.sql` (Ops #300, Konsey `konsey/tutanak/2026-09-21-ik-suche-kassenfeld.md`)
- **Status:** ⚠️ **Datei geschrieben, LIVE NICHT ANGEWANDT** (Stand beim Schreiben; der Supabase-MCP war nicht autorisiert, Melih wendet an). Erst danach hat die IK-Suche etwas zu lesen. Reihenfolge bindend: **Sicht live, dann Frontend deployen.**
- **Wer:** `module/krankenkasse-suche.js` — `sucheKassenfeld()` → `holeKostentraeger()` fragt pro Eingabe (≥ 3 reine Ziffern) serverseitig nach Präfix (`like 'ziffern%'`, `order ik`, `limit 300`). Fehlt die Sicht, fängt das Modul es ab (eine Warnung, leere Liste, Namenssuche unberührt). Geschrieben wird nie.
- **Achtung — die Zeilenzahl ist am Seed nachgerechnet, nicht live gemessen:** 1.043 → **893** (82 ohne VKG-Segment, 68 abgelaufen), keine der bekannten Rechenzentren drin. Rund ein Dutzend der 82 ausgeschlossenen trägt einen schlichten Kassennamen (AOK NORDWEST, DAK-Gesundheit ×2, Techniker Krankenkasse …): wer eine solche IK abtippt, findet nichts (Freitext bleibt möglich, falsche Daten entstehen nicht). Offene Frage an `gkv-302`, ob das Karten-IKs sein können. Der Seed führt für die Ersatzkassen noch `EK05Q426` (Q4), live ist Q2 — die Live-Zahl weicht ab. Vorab-/Nachher-Abfragen stehen im Kopf der Migration.
- **Achtung — Rechte:** `security_invoker` (erbt die RLS von `kostentraeger` und `kostentraeger_annahmestellen`, nur `authenticated`), ausdrücklich `REVOKE ALL … FROM PUBLIC, anon, authenticated, service_role`, danach `GRANT SELECT` nur an `authenticated`. Die zwei Baseline-Sichten (`profiles_public`, `fahrten_monthly_summary`) tragen dagegen noch die Supabase-Standardrechte (u. a. `GRANT ALL … TO service_role`, bei `fahrten_monthly_summary` sogar Schreibrechte für `anon`) — sie stehen in diesem Register nicht.
- **Achtung — still verlorene Zeilen:** Der `EXISTS`-Teil wird ebenfalls durch RLS geprüft. Verschärft jemand `kostentraeger_annahmestellen_read_all`, verliert die Sicht Zeilen, ohne Fehler.
- **Achtung — Sichten sind für die Zähler und die Tabellenkarte unsichtbar:** `schema-zaehler.js` zählt nur `BASE TABLE`, `tools/tabellenkarte.mjs` matcht nur `CREATE TABLE`. Der Leser steht deshalb **nicht** in `NUTZUNG.json` — dieser Eintrag ist die einzige Spur. Folgeänderungen an der Sicht dürfen Spalten nur **anhängen**, sonst DROP + CREATE.
- **Nicht in `api/dsgvo.js`** — Referenzdaten ohne Personenbezug, keine Tabelle.
...
- **Warum:** Anlage 1 TP5 Kap. 3(2) schreibt vor: *„Über den Datenaustausch ist eine Dokumentation zu führen … mindestens **2 Jahre** aufzubewahren … **alle Schritte von der Initiierung bis ggf. zur Quittierung**."* Anhang 1 § 4.5(2) zählt zehn Mindestfelder auf (physikalischer Dateiname, Erstellungsdatum, lfd. Nr., Kommunikationspartner, Beginn/Ende, Dateigröße, Verarbeitungshinweise, Senden/Empfangen, Verarbeitungskennzeichen, Fehlerstatus). **Diese Dokumentation lässt sich nicht nachträglich erzeugen** — deshalb entsteht die Tabelle, bevor der Versandschritt gebaut wird, und nicht danach.
  **Warum eine eigene Tabelle und keine Spalten an `abrechnung`** (`db-ustasi`, 20.09.2026): die Kardinalität ist **1:n** — eine Abrechnung kann mehrfach übertragen werden, und Quittungen kommen zeitversetzt zurück (`antwort_auf`). ⚠️ **Korrektur 22.09.2026:** die ursprüngliche Begründung nannte hier zusätzlich, `abrechnung` sei festgeschrieben — **das ist falsch** und stand so auch in `0034`. Auf dieser Tabelle liegen nur `abrechnung_updated_at` und `trg_set_business_id`, die `signed_*`- und `encrypted_*`-Gruppen werden dort sehr wohl nachträglich per UPDATE gefüllt. Die Entscheidung für eine eigene Tabelle bleibt richtig, aber **allein wegen der 1:n-Kardinalität** — nicht wegen eines GoBD-Riegels, den es dort nicht gibt.
- **Seit:** 20.09.2026 · `api-backend/db/migrations/0034_abrechnung_uebermittlung.sql` (ABRECHNUNG_ECHTBETRIEB_PLAN.md, Schritt 1.10)
- **Status:** aktiv, aber **noch nicht angeschlossen** — ✅ Migration am 20.09.2026 live angewandt (MCP); der Versandweg selbst entsteht erst in Faz 2, die Tabelle ist deshalb leer. Die Tabelle und ihre Schreibfunktion existieren vorab, damit niemand die Dokumentation später rückwirkend erfinden muss.
- **Wer:** `uebermittlungProtokollieren()` in `api-backend/billing/api/uebermittlung.js` schreibt — heute von keiner Route aufgerufen, Anschluss mit dem Versandschritt. Gelesen wird sie künftig vom Inhaber über seine eigene Oberfläche.
- **Achtung — festgeschrieben, Bauart wie `abrechnung_zeile`.** `fn_abrechnung_uebermittlung_festschreibung()` wirft bei **jedem** DELETE bedingungslos. Gesperrt sind alle Stammdaten des Transports (Richtung, Dateiname, Erstellungsdatum, laufende Nummer, Transfernummer, Partner-IK, Absender-IK, Beginn, Dateigröße, SHA-256, Betriebsart, Mandant, Abrechnungsbezug, Anlagedatum). **Nur einmalig ergänzbar** (NULL → Wert): `beendet_am`, `antwort_auf`, `uebertragungsweg`. Frei: `verarbeitungskennzeichen`, `fehlerstatus`, `fehlertext`, `partner_name` — und **`verarbeitungshinweise` vollständig frei**, weil es in der Kundenbox keinen SQL-Zugang gibt (O-114): eine Korrektur muss dort eine **zusätzliche Notiz** sein können, sonst gibt es gar keinen Weg.
- **Achtung — RLS lässt den Inhaber ausdrücklich LESEN.** Genau eine Policy, `FOR SELECT`, nach dem Muster von `Abrechnungszeile select scoping`; **keine** INSERT/UPDATE-Policy, geschrieben wird nur mit `service_role`. Die Tabelle ist bewusst **nicht** wie `nummernkreise` komplett verschlossen — Bedingung von `onprem`: in der Kundenbox muss der Inhaber die Frage *„wann ist diese Datei rausgegangen?"* an seinem **eigenen** Bildschirm beantworten können, wir kommen dort nicht hinein.
- **⛔ Achtung — striktes PHI-Verbot.** Hier stehen Transportdaten, keine Akte: **kein** Patientenname, **keine** Versichertennummer, **kein** Geburtsdatum, **keine** Diagnose. Die zwei Risikostellen sind die Freitextfelder `verarbeitungshinweise` und `fehlertext` — wird dort je eine ZAA-Rückmeldung hineinkopiert, muss sie **vorher** maskiert werden.
- **In `api/dsgvo.js`: Auskunft ja, Löschung nein.** `USER_TABLES` mit `owner_id`; im ⛔-Block mit der 2-Jahres-Frist begründet, gleiche Kategorie wie `abrechnung` und `abrechnung_zeile`. ⚠️ Ehrliche Folge: `owner_id` steht auf `ON DELETE RESTRICT` — solange solche Zeilen stehen, lässt sich die `profiles`-Zeile nicht löschen, derselbe Zustand wie bei `belegliste` und `patient_consents`.
...
- **Achtung — nicht neu verdrahten:** Der Tabellenname klingt nach „hier stehen die gültigen Preise". Er tut es nicht. **Preisquelle für Physio ist `billing/codes/physio_positions.js`, für Podologie `billing/codes/podologie_positions.js`.** `heilmittel_katalog` ist eine reine UI-Projektion dieser beiden Dateien (`sync_heilmittel_katalog.js`), `resolvePreis()` liest sie nie. Wer die Tabelle wieder an den Resolver hängt, baut denselben Fehler erneut ein. (Korrektur 14.09.2026, db-ustasi-Review — hier stand vorher fälschlich `heilmittel_katalog` als Podologie-Preisquelle, derselbe Fehlerklasse wie O-96.)
...
- **Status:** aktiv (Referenz) — `ik_number` ist eine **Vorbelegung, nur wenn eindeutig**, keine Abrechnungswahrheit (Ops #301, Konsey 21.09.2026). Live 21.09.2026: 76 von 94 gefüllt, 18 NULL. Nach Migration `0041_krankenkassen_ik_nachtrag`: 74 gefüllt, 20 NULL — ⏳ **vorbereitet, noch nicht angewandt** (beim Anwenden diese Zeile auf ✅ mit Datum ändern).
- **Wer:** Lesen — genau drei Stellen (`fonksiyon-ustasi`, 21.09.2026): `module/krankenkasse-suche.js` (Auswahlfeld; füllt das Geschwisterfeld `<id>Ik` nur, wenn dort noch nichts steht), `loadKkList()` in `dashboard.js` (`rzKkList`, setzt `rzPatKasseIk` per exaktem Namensvergleich) und `GET /api/krankenkassen` in `server.js` (einziger Konsument `booking-request.js` nutzt nur `id`/`name` und verwirft die IK). Schreiben — nur die Migration `0009_seed_krankenkassen.sql`; keine Admin-Oberfläche, kein Skript.
- **Achtung:** **Nicht** `kostentraeger`. Diese hier ist die Oberfläche, jene die §302-Seite.
- **Achtung — „zentral oder regional?" ist die falsche Frage (Ops #301, Konsey 21.09.2026, `gkv-302`):** Der DTA kennt zwei IK-Pflichtfelder. Die **Karten-IK** ist der Aufdruck der Versichertenkarte; sie steht auf Muster 13 im Feld Kostenträgerkennung und darf nicht handschriftlich geändert werden (Podologie Anlage 3 „notwendige Angaben", Abschn. a). Die **Kostenträger-IK** ist die IK, auf die die Karten-IK in der Kostenträgerdatei per `VKG`-Verknüpfungsart 01 verweist (Anlage 1 TP5 V21 § 5.5.3.1; Anhang 3 zu Anlage 1 TP5 V10 § 5.1). In der Datei Q3/2026 zeigen 992 Karten-IKs auf eine andere IK, nur 225 auf sich selbst. Beide Bauformen stehen nebeneinander: AOK Bayern bündelt 40 Karten-IKs auf eine zentrale `108310400` (als Kostenträger-IK im DTA korrekt, auf Muster 13 steht sie nicht), AOK Baden-Württemberg hat **14 gleichrangige Bezirks-IKs und keine zentrale**. Eine pauschale Regel erzeugt bei einer der beiden Kassen eine falsche Datei. **Entscheidungsregel:** Eine IK steht in `ik_number` nur, wenn die Kasse genau **einen** Kostenträger hat; bei mehreren bleibt NULL, denn die richtige IK hängt an der Karte des einzelnen Patienten. Der Wert landet über `loadKkList()`/`podNewKk` in `prescriptions.kostentraeger_ik` und im DTA — eine geratene IK bedeutet Dateiabweisung (Prüfstufe 3) oder Fehlleitung an die falsche Datenannahmestelle.
- **Bestandsprüfung 21.09.2026** (alle 76 gefüllten IKs gegen die 7 Dateien der Kostenträgerdatei Q3/2026): 67 sind Kostenträger und gehören namentlich zur Kasse; **9 existieren in der Datei gar nicht** (Reste des alten Mock-Seeds, die #264 nicht erfasst hatte): AOK Nordost `101000016`, IKK classic `107708612`, Mobil `107300000`, AOK Niedersachsen `108310401`, AOK Rheinland/Hamburg `107436001`, AOK PLUS `101000026`, BAHN-BKK `101317994`, BIG direkt gesund `107636345`, Landwirtschaftliche Krankenkasse `109006429`. Die vier falschen Werte aus der Prüfung vom 06.09.2026 (DAK, hkk, KKH, AOK BW) sind seit #264 (Commit `cceb528`) korrigiert bzw. NULL.
- **Vorgehen — Migration `0041_krankenkassen_ik_nachtrag.sql`** (13 Zeilen, per `id`, nur Daten; ⏳ vorbereitet, noch nicht angewandt): **nachtragen (4):** AOK Hessen `105313145`, DIE BERGISCHE KRANKENKASSE `104926702`, BKK firmus `102529638` (Datei: „firmus/Ost", einziger Kostenträger, zwei weitere Sätze verweisen darauf), Krones BKK `108934142`. **ersetzen (3):** Mobil `107300000` → `101520078`, AOK Niedersachsen `108310401` → `102114819` (52 Karten-IKs verweisen darauf), AOK PLUS `101000026` → `107299005` (Satz „DAV AOK Plus Sachsen", gleiches Muster wie AOK Bayern, von `gkv-302` bestätigt). **leeren (6):** AOK Nordost, IKK classic, AOK Rheinland/Hamburg, BAHN-BKK, BIG direkt gesund, Landwirtschaftliche Krankenkasse → NULL (je mehrere Kostenträger). Eine Zeile wird nur angefasst, wenn sie NULL ist oder den erwarteten Altwert trägt; danach drei Selbstprüfungen (Sollwert je Zeile · jede geschriebene IK existiert in `kostentraeger` mit `datensatz_status = 'echt'` · keine der 9 Mock-IKs übrig), sonst läuft die Datei nicht. In der Kundenbox (`0009` = überall NULL) werden nur die 7 IKs eingetragen. Rollback: keine `down`-Datei (README Kural 3) — die 9 Altwerte stehen im Kopf der Migration, der Rückweg ist eine neue Datei.
- **Bleiben NULL, begründet (20 Zeilen):** 18 mit mehreren echten Kostenträgern (AOK Baden-Württemberg 14 · AOK Nordost 3 · Landwirtschaftliche Krankenkasse 8 · je 2: AOK NordWest, IKK - Die Innovationskasse, IKK Brandenburg und Berlin, IKK gesund plus, Koenig & Bauer BKK, mhplus, mkk, SBK, SECURVITA, vivida bkk, ZF BKK, IKK classic, AOK Rheinland/Hamburg, BAHN-BKK, BIG direkt gesund) und 2 nicht GKV („Andere", „Privatversicherung": §302 gilt nicht, NULL ist fachlich richtig). Ein „alle 94 belegt" gibt es deshalb bewusst nicht. **Fertig-Kriterium:** jede Zeile trägt eine in `kostentraeger` existierende IK oder ein begründetes NULL. Kein NOT NULL; nur Vorschlag (nicht gebaut): `CHECK (ik_number IS NULL OR ik_number ~ '^[0-9]{9}$')`.
- **Auflösung, wenn das Formular keine IK mitschickt:** `kostentraegerIkAufloesen` (`api-backend/lib/rezept-felder.js`, Commit `fa86f5b`) nahm bei mehreren Namenstreffern einen zufälligen Satz (`limit(1)` ohne `order`; gemessen: 28 von 94 Kassennamen mehrdeutig). Jetzt werden alle Treffer über `abrechnender_kt_ik` bis zum Kostenträger zurückgeführt (auch Ketten); genau ein Kostenträger ergibt dessen IK, sonst `null` (Messung gegen den Seed `0006`: 14 eindeutig, 80 `null`). ⚠️ Restschwäche (Folge-Ticket): Die Teilstring-Suche kann eine fremde Kasse treffen („BKK Salzgitter" → „Public - Partner der BKK Salzgitter"); im Normalfall schickt das Auswahlfeld die IK mit.
- **Offen / Folge-Tickets (nicht in #301):** AOK BW und die anderen Mehr-IK-Kassen für Beta-1 abrechenbar machen (Karten-IK je Patient beim Erfassen; kleinste Version: das vorhandene Feld `rzPatKasseIk` bei Mehr-IK-Kassen als Pflicht, mit klarer Meldung schon beim Erfassen) · Karten-IK-Modell (`prescriptions.krankenkasse_ik` wird nirgends geschrieben; Fallback `krankenkasseIk || kostentraegerIk` in `billing/dta/builder.js`) · Härtung der Namenssuche. Beschluss: `konsey/tutanak/2026-09-21-krankenkassen-ik-nachtrag.md`.
- **Quelle:** `wissensbank/gemeinsam/kostentraeger/*.txt` (Q3/2026: `AO05Q326_KE3`, `BK05Q326_KE1`, `IK05Q326_KE1`, `LK05Q226_KE0`, …), Anlage 1 TP5 V21 § 5.5.3.1, Anhang 3 V10 § 5.1 und § 8.13, `wissensbank/REGISTER.md` W-01.
...
- **Teamzugriff (seit 17.09.2026, Migration `0023_aerzte_ausfall_team_insert`, Ops #299):** Angestellte dürfen **lesen, anlegen und ändern** — **löschen nicht** (`aerzte_delete_owner` bleibt beim Inhaber). Auslöser war die service_role-Prüfung S-30: `resolveOrCreateArzt()` (`api-backend/lib/arzt-registry.js`) schreibt im Rezept-Scan mit dem Service-Role-Client und prüft keine Rolle — der Angestellte, der scannt, legte den Arzt also längst an, RLS sagte nur etwas anderes. Statt die Route auf 403 zu setzen (das hätte den täglich laufenden Scan-Ablauf gebrochen, der Bildschirm ist in `nav-registry.js` für `owner`+`employee` freigegeben) wurde RLS an das gewollte Verhalten angeglichen. **UPDATE ist bewusst dabei:** der Normalfall in `resolveOrCreateArzt()` ist die Anreicherung eines vorhandenen Eintrags bei LANR-Treffer, nicht die Neuanlage — ohne UPDATE wäre das INSERT-Recht halb wertlos. Vertretbar, weil `aerzte` **Stammdatenregister** ist (Name, LANR, BSNR, Anschrift), kein Art.-9-Patientendatum und keine Dokumentation nach § 630f BGB. Löschen bleibt beim Inhaber, weil ein gelöschter Arzt die Auswertung „welcher Arzt überweist wie viel“ aufreißt und aus der Verordnung heraus nicht wiederherstellbar ist.
...
- **Seit:** 22.05.2026 · `v26_data_access_log` (DB-Schreibtrigger nachgezogen: 17.09.2026 · `0021_audit_write_trigger`, Ops #254)
- **Status:** aktiv
- **Wer:** Zwei Schreiber, sonst keiner. (1) `api-backend/_lib/access-log.js` → `logAccess()` mit service_role — die HTTP-Ebene (`method` = GET/POST/…, `path` = echter Request-Pfad). (2) Seit 17.09.2026 `audit_write_log()`, ein SECURITY-DEFINER-Trigger auf `leads`, `prescriptions`, `podologie_behandlungen` — die DB-Ebene (`method='DB'`, `path='db://<tabelle>'`, `action`=INSERT/UPDATE/DELETE). Gelesen wird über die DSGVO-Auskunft und die eine SELECT-Policy des Inhabers.
- **Achtung — warum es überhaupt einen DB-Trigger braucht (Ops #254, Sicherheitsregister A-18):** `access-log.js` sieht nur, was durch den Express-Backend läuft. Das Dashboard schreibt Patientendaten aber grösstenteils **direkt per PostgREST** — diese Schreibvorgänge liefen bis 17.09.2026 an jedem Protokoll vorbei. `guvenlik` hat gegen `information_schema.triggers` belegt, dass die drei Patientendaten-Tabellen keinen einzigen Audit-Trigger hatten.
- **Achtung — `metadata` enthält NUR Spaltennamen, nie Werte.** Bei den DB-Zeilen steht dort `{"geaenderte_spalten": [...]}`. Ein Log mit alten/neuen Werten wäre eine zweite Patientenakte, die in keiner Löschkette hängt (Befund R12 im Sicherheitsregister). Was drinstand, beantwortet der Datensatz selbst — und nur der steht in `api/dsgvo.js`.
- **Achtung — Lesezugriffe (SELECT) werden bewusst NICHT protokolliert** (`guvenlik`, 17.09.2026): RLS hält die Mandantengrenze bereits, und ein pgaudit-Leseprotokoll trüge PHI in ein neues Silo. Der Beweiswert (§ 630f BGB, GoBD) steckt in der Schreibseite.
- **Achtung — die Tabelle hat mit Absicht nur eine SELECT-Policy.** Kein Client darf hier einfügen, ändern oder löschen; `authenticated` hat zwar das INSERT-GRANT, scheitert aber an der fehlenden Policy (live geprüft 17.09.2026: Fehler 42501). Genau deshalb ist der Trigger `SECURITY DEFINER` — der Definer umgeht RLS, der Client nicht. **Wer hier eine INSERT-Policy ergänzt oder `FORCE ROW LEVEL SECURITY` setzt, macht das Protokoll fälschbar bzw. schaltet den Trigger tot.**
- **Achtung — das Protokoll blockiert nie einen Schreibvorgang.** `audit_write_log()` ist in `EXCEPTION WHEN OTHERS THEN RETURN COALESCE(NEW, OLD)` gefasst: fällt das Log-INSERT aus, fehlt der Eintrag, aber die Behandlungsdokumentation wird geschrieben. Live mit sabotiertem Audit-INSERT geprüft.
- **Achtung — `user_id` ist NULL, wenn der Backend mit service_role schreibt** (kein JWT-`sub`). Das ist die Trennlinie, kein Defekt: `user_id` gefüllt = der Browser des Behandlers hat direkt geschrieben; leer + `method='DB'` = es kam über den Backend. `owner_id`/`resource_id` stehen in beiden Fällen.
