...
- **Achtung — der Weg zur Nummer führt seit 11.09.2026 nur noch über den Trigger.** `naechste_nummer(uuid, text, integer)` und `naechste_verordnungsnummer(uuid, uuid)` haben kein EXECUTE mehr für `anon`/`authenticated`/`PUBLIC` (`api-backend/db/migrations/0002_nummernkreis_rpc_revoke.sql`, Fund S-24). Beide nahmen den Mandanten als Argument entgegen und prüften ihn nicht gegen `auth.uid()` — ein `POST /rest/v1/rpc/naechste_nummer` mit fremder `owner_id` verbrauchte eine Nummer einer fremden Praxis und riss deren lückenlose Nummerierung (§ 146 Abs. 4 AO) von außen auf. **Die Vergabe selbst ist unberührt:** die einzigen zwei Aufrufer, `set_invoice_nummer()` und `vergebe_verordnungsnummer_rx()`, sind SECURITY DEFINER und laufen als Funktionsbesitzer (live `postgres`, in der lokalen Box `supabase_admin`), nicht unter der Aufruferrolle. Am 11.09.2026 in der Box mit echten INSERTs nachgewiesen statt nur angenommen: `INV-2026-0001/0002/0003`, eigener Kreis ab `issued_at` 2027, Verordnungsnummern 1/2, UPDATE-Einfrierung hält, Direktaufruf als `anon` und `authenticated` scheitert mit `permission denied for function`. **Ein künftiger Aufrufer muss ebenfalls SECURITY DEFINER sein** — ein SECURITY-INVOKER-Aufrufer bekommt seit 0002 `permission denied`.
- **Achtung — gefüllt wird nur der Kreis `rechnung`.** Die Zeile oben nennt `beleg_nr` und `mahnung_nr` mit; das war die Absicht von 16.08.2026, eingelöst wurde sie nie. Am 11.09.2026 nachgezählt: `SELECT DISTINCT kreis FROM nummernkreise` liefert genau einen Wert, `rechnung`. `belegliste.beleg_nr` und `mahnungen.mahnung_nr` kommen aus `set_next_beleg_nr()` bzw. `set_next_mahnung_nr()` — beide zählen mit `SELECT COALESCE(MAX(…),0)+1` je `owner_id` hoch, also mit genau dem `MAX+1`, dessen Rennen diese Tabelle abschaffen sollte. Zwei gleichzeitige Beläge derselben Praxis können dieselbe Nummer bekommen (der `UNIQUE`-Index fängt es als Fehler ab, vergibt aber keine zweite Nummer). Beide Trigger sind zudem SECURITY INVOKER — sie rufen `naechste_nummer` nicht auf und sind von 0002 daher **nicht** betroffen. Offene Produktfrage, kein Regressionsfund: umstellen oder die Absicht hier streichen.
