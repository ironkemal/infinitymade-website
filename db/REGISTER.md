...
- **Achtung — „82 ohne Routing" ist die freundliche Zahl, die harte ist 125:** von den 302 abrechnenden Kostenträgern haben **82** überhaupt kein VKG-Segment (überwiegend AOK-NORDWEST-Regional-IKs und die Verbands-IKs der `gkv informatik`). Für unseren Weg zählt aber nur eine elektronische Datenannahmestelle (Verknüpfungsart 02/03 **und** `art_datenlieferung` 07/30) unter einem für uns brauchbaren Abrechnungscode. Danach sieht es so aus: **87** auflösbar direkt über 71/72 · **90** nur über den Sammelschlüssel 00 · **43** haben zwar Segmente, aber keines mit 71/72/20/00 · **82** haben gar keins. Zusammen **177 auflösbar, 125 nicht**. Wer die Tabelle anschliesst, braucht also zwingend eine Fallback-Kette 71/72 → 20 → 00 — und trotzdem einen definierten Ausweg für die restlichen 125.
...
- **Achtung — die Abfrage ist nicht überall eindeutig:** über alle Abrechnungscodes hinweg liefern **14 von 2.309** Schlüsseln (`kostentraeger_ik, abrechnungscode, art_datenlieferung, bundesland`) **zwei** Empfänger-IKs — durchweg `103411401` ↔ `104212516` (gkv informatik, Bundesland 05) unter Sammelschlüssel 00. Für die Codes **20/71/72 ist die Abfrage eindeutig** (264 Schlüssel, 0 mehrdeutig). Ein `.maybeSingle()` auf die Fallback-Stufe 00 kann also scheitern — das muss der erste Leser abfangen.
