...
- **Achtung — `letzte_referenz` ist `bigint` und läuft NICHT über.** Das UNB-Feld ist fünfstellig; der Fünfsteller entsteht erst bei der Ausgabe (`((n-1) % 99999) + 1`). So überläuft das Spezifikationsfeld, die Historie aber nicht. `letzte_transfernummer` läuft dagegen wirklich im Kreis, 1..999.
...
- **Offene Fachfrage an `gkv-302`:** Der Plan schreibt für die Transfernummer „0..999", der in `filename.js` zitierte Wortlaut aus Anhang 1 § 4.3 „1..999". Die Migration folgt dem Zitat (1..999). Falls 0 zulässig ist, ist das eine reine Erweiterung des Wertebereichs.
