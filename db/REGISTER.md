...
- **Achtung — `quelle_stand` fehlte bis 20.09.2026 im Seed-Generator.** `tools/seed-generieren.mjs` führte für diese Tabelle `quelle` in `columns`, aber nicht `quelle_stand` (beim Nachbarn `kostentraeger` stand es drin). Folge: das ausgelieferte `0007_seed_kostentraeger_annahmestellen.sql` schrieb in jeder Kundenbox `quelle_stand = NULL` — und dort ließ sich genau die Frage nicht mehr beantworten, um die es im Zeitfehler oben geht: **wie alt ist diese Empfängerliste?** Die Spalte ist ergänzt; die Korrektur in der Box kommt erst mit einer neu erzeugten Seed-Migration (braucht einen Lauf gegen die Live-DB).
...
- **Status:** aktiv (Referenz) — ✅ **am 20.09.2026 live angewandt** (MCP). Befüllt wird sie von `tools/kostentraeger-annahmestellen-laden.mjs --write` (derselbe Lauf wie die VKG-Zeilen); die Seed-Migration für die Kundenbox entsteht daraus über `tools/seed-generieren.mjs`. ZEILENSTAND_PLATZHALTER
