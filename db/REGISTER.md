...
- **Seit:** 07.09.2026 · `sql-melih/2026-09-07-rechnung-zahlungen.sql` — ⚠️ **direkt im SQL-Editor gefahren, steht in KEINER Migrationszeile** von `supabase_migrations.schema_migrations` (das endet am 05.09.2026). Live ist sie trotzdem, am 08.09.2026 gegen die DB geprüft.
- **Status:** aktiv
...
- **Achtung:** Append-only — Trigger `prevent_rechnung_zahlungen_mod()` blockt UPDATE/DELETE. Korrektur nur durch **neue** Zeile mit `art='storno'` und negativem Betrag; der UNIQUE-Index auf `storniert_zeile_id` macht jede Zeile genau einmal stornierbar.
  - **Nicht von Hand INSERTen.** `rechnung_zahlung_buchen()` ist eine Transaktion: Zahlungszeile, je nach Restbetrag eine Ausbuchungszeile, bei Barzahlung die `belegliste`-Zeile (`invoice_id`), ggf. Fortschreibung in `prescriptions` und das Nachziehen von `invoices.payment_status`. Wer direkt schreibt, bekommt eine Zahlung ohne Beleg und einen falschen Rechnungsstatus.
  ...
  - **DSGVO (08.09.2026):** Beim Anlegen stand die Tabelle **weder** in `USER_TABLES` noch im ⛔-Block von `api/dsgvo.js` — genau der Fehler vom 28.08.2026, ein zweites Mal.
    - ✅ **Auskunft (Art. 15) nachgetragen** — `{ table: 'rechnung_zahlungen', filter: 'owner_id' }` steht jetzt in `USER_TABLES`. Ohne sie fehlte die Antwort auf „was habe ich wann auf welche Rechnung gezahlt", seit `invoices.payment_status` nur noch Cache ist.
    - ⏳ **Löschung (Art. 17) offen, liegt bei `legal-de`.** `owner_id` zeigt mit **RESTRICT** auf `profiles` — die Tabelle sperrt die Profillöschung genau wie `belegliste`; `invoice_id` (ebenfalls RESTRICT) sperrt zusätzlich das Löschen einer einzelnen Rechnung, sobald darauf gezahlt wurde. Löschen, anonymisieren (wie `invoices`) oder aufbewahren (wie `belegliste`) ist eine Abwägung GoBD/§ 147 AO gegen Art. 17 Abs. 3 lit. b. Bis zur Entscheidung steht sie im ⛔-Block von `api/dsgvo.js` vermerkt, damit sie nicht ein drittes Mal übersehen wird.
...
| `rechnung_zahlungen.owner_id` (RESTRICT) — **neu 07.09.2026, in `api/dsgvo.js` noch NICHT vermerkt** | Profil lässt sich nicht löschen, solange eine Zahlung gebucht ist | Dasselbe Muster wie `belegliste`: gebuchter Zahlungsvorgang, GoBD / § 147 AO. `invoice_id` steht ebenfalls auf RESTRICT — auch eine einzelne Rechnung ist nicht mehr löschbar, sobald darauf gezahlt wurde |
