...
- **Achtung — fünf neue Spalten am 22.09.2026: die Verschlüsselung** (`0039_abrechnung_verschluesselung`, live angewandt). `verarbeiteVerschluesselungsSchritt()` erzeugte die CMS-EnvelopedData-Datei und gab sie **nur im HTTP-Response** von `/upload-signed` zurück — nirgends gespeichert. Nach einem Reload wusste niemand mehr, ob eine verschlüsselte Datei überhaupt existiert; der Download-Knopf bot deshalb ausschliesslich die **unverschlüsselte** signierte Datei an, und ein fehlendes Empfängerzertifikat blieb dem Inhaber unsichtbar (O-131, Kaltprüfung 21.09.2026).
