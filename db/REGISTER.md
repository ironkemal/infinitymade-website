...
- **Status:** aktiv — ⚠️ **Migration wartet zum Zeitpunkt dieses Eintrags noch auf die Anwendung in der Live-Datenbank.**
...
- **Achtung — RLS ist an, es gibt bewusst KEINE Policy.** Gleiche Bauart und gleicher Grund wie `nummernkreise`: kein Client fasst den Zähler an. Die Funktionen haben `REVOKE … FROM PUBLIC` und `GRANT … TO service_role` — die Falle aus S-24 (Mandant kommt als Argument und wird nicht gegen `auth.uid()` geprüft) ist hier von Anfang an zu, nicht nachträglich. `tools/check-security-definer-grants.mjs` bewacht das.
...
- **Status:** aktiv — ⚠️ Migration wartet noch auf die Anwendung in der Live-Datenbank.
...
- **Status:** aktiv (Referenz) — ⚠️ **Migration wartet noch auf die Anwendung in der Live-Datenbank, und die Tabelle ist noch LEER.** Befüllt wird sie von `tools/kostentraeger-annahmestellen-laden.mjs --write` (derselbe Lauf wie die VKG-Zeilen); die Seed-Migration für die Kundenbox entsteht danach aus `tools/seed-generieren.mjs`.
...
- **Status:** aktiv, aber **noch nicht angeschlossen** — ⚠️ Migration wartet auf die Anwendung, und der Versandweg selbst entsteht erst in Faz 2. Die Tabelle und ihre Schreibfunktion existieren vorab, damit niemand die Dokumentation später rückwirkend erfinden muss.
