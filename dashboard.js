...
import { mountPodologieAbrechnung, setPodVorwahl, getPodVerordnung } from './module/podologie-abrechnung.js?v=20260920u';
...
async function switchPanel(id) {
  ...
  if (id === 'podologie-billing') mountPodologieAbrechnung(podoCtx());
...
// Heilmittel/Rezeptart blocks removed — keep stubs for callers
...
async function pdSpringeZu(ziel, id, extra = {}) {
  ...
  if (ziel === 'podologie') setPodVorwahl(id);                     // Verordnung vorgewählt
...
// Hier standen ~400 Zeilen: ein ZWEITES Fussbefund-Formular fuer die Patientenkarte.
// Es schrieb in dieselbe Tabelle wie `module/fussbefund.js`, aber ohne `booking_id`
// und ohne `uebernommen_von` — ein hier gespeicherter Befund stand im Modul als
// "Ohne Termin" und der Termin bekam kein "Befund"-Haken. Entfernt 27.08.2026;
// die Patientenkarte zeigt nur noch ein Archiv (module/fussbefund-archiv.js),
// erfasst und geaendert wird ausschliesslich im Modul unter "Fussbefund".

/** Archiv in der Patientenkarte zeichnen. Nur lesen — siehe Modulkopf. */
function zeigeFussbefundArchiv(leadId) {
  return renderFussbefundArchiv({
    supabase,
    escapeHtml,
    oeffneEintrag: ({ leadId: lid, befundId }) =>
      oeffneFussbefundEintrag(fussbefundCtx(), { leadId: lid, befundId }),
  }, leadId);
}
...
// Apify/Google-Maps-Suche (b2b + Ärzte "In der Nähe finden") → module/lead-suche.js
// (O-09, onprem/REGISTER.md — On-Prem-Box entfernt diese Knoten statt sie zu laden)
...
const { loadDoctors } = initLeadSuche({
  istKutu: IST_KUTU, supabase, showToast, t, getOwnerId, loadB2B, renderB2B,
});
...
// ============================================================================
// PODOLOGIE-ABRECHNUNG → module/podologie-abrechnung.js
//                        module/diagnosegruppen-regeln.js
// ----------------------------------------------------------------------------
// Der ~1270-Zeilen-Block ist am 27.08.2026 umgezogen (Konsey 2026-08-13:
// „angefasst heisst umgezogen"). Reiner Umzug, Verhalten unverändert.
// Hier bleibt nur die Verdrahtung — podoCtx() steht neben fussbefundCtx().
//
// Eine Ausnahme: rechnungAusVerordnung() steht weiterhin hier. Sie SCHREIBT in
// invLines / invVerordnungId / invBehandlungIds / invPatientInsuranceType;
...
/**
 * Abhängigkeiten für module/podologie-abrechnung.js.
 *
 * ⚠ `leads` und `services` MÜSSEN Getter bleiben. `leadsCache` und
 * `ownerServices` sind hier `let` und werden neu zugewiesen; als Wert
