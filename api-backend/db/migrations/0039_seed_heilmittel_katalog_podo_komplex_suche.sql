-- Ops #302 — „Komplex" soll in der Heilmittelsuche 78010 und 78020 finden.
--
-- Beta-1 (18.09.2026): Auf der Verordnung steht „podologische Komplexbehandlung",
-- im Katalog heißen die Positionen „Podologische Behandlung (klein/groß)" —
-- amtlich korrekt (Anlage 1a Teil 2 Ziff. 3, Anlage 2 §2/§3), aber ohne das Wort.
-- „Komplexbehandlung" ist das VERORDNETE Heilmittel c) (HeilM-RL § 27a Abs. 4
-- Nr. 3), keine Leistung; sie wird mit 78010 (bis 20 Min) ODER 78020 (über 20 Min)
-- abgerechnet (Anlage 1a Teil 1 Nr. 4, FAK Podologie Q25). Deshalb bekommen BEIDE
-- Positionen in beiden Preisfenstern (vier Zeilen) den Such-Anker in `kategorie`;
-- `label` bleibt unverändert. search_heilmittel() durchsucht `kategorie` bereits
-- (LIKE, Rang 40) — keine RPC-/Schemaänderung.
-- `kuerzel` bleibt bewusst NULL: katalog-suche.js zeigt `kuerzel || code`, die
-- Positionsnummer verschwände sonst aus der Auswahlliste.
--
-- Erzeugt mit `node api-backend/sync_heilmittel_katalog.js --sql` (Vollausgabe wie
-- 0012/0015, kein zweiter Erzeugungsweg). Gegenüber der Ausgabe vor der Änderung
-- weichen genau diese vier Zeilen ab, sonst nichts (diff geprüft).
--
...
INSERT INTO public.heilmittel_katalog (code, bereich, label, kuerzel, kategorie, diagnosegruppen, preis_eur, zuzahlung_eur, dauer, gueltig_ab, gueltig_bis, deprecated, ungueltig_ab, ersetzt_durch, max_pro_tag, max_pro_termin, notiz, gruppe, telemed, sort) VALUES
  ...
  ('78010','podologie','Podologische Behandlung (klein)',NULL,'Podologische Komplexbehandlung','{DF,NF,QF}','35.16','3.52','35','2025-07-01','2026-06-30','f',NULL,NULL,NULL,NULL,NULL,'f','f','0'),
  ('78020','podologie','Podologische Behandlung (groß)',NULL,'Podologische Komplexbehandlung','{DF,NF,QF}','50.55','5.06','50','2025-07-01','2026-06-30','f',NULL,NULL,NULL,NULL,NULL,'f','f','1'),
  ...
  ('78010','podologie','Podologische Behandlung (klein)',NULL,'Podologische Komplexbehandlung','{DF,NF,QF}','36.1','3.61','35','2026-07-01','9999-12-31','f',NULL,NULL,NULL,NULL,NULL,'f','f','18'),
  ('78020','podologie','Podologische Behandlung (groß)',NULL,'Podologische Komplexbehandlung','{DF,NF,QF}','51.92','5.19','50','2026-07-01','9999-12-31','f',NULL,NULL,NULL,NULL,NULL,'f','f','19'),
...
DO $$ DECLARE n int; k int; BEGIN
  ...
  SELECT count(*) INTO k FROM public.heilmittel_katalog
   WHERE bereich = 'podologie' AND code IN ('78010','78020')
     AND kategorie = 'Podologische Komplexbehandlung';
  IF k <> 4 THEN RAISE EXCEPTION 'seed heilmittel_katalog: % Komplex-Anker fuer 78010/78020, erwartet 4', k; END IF;
END $$;
