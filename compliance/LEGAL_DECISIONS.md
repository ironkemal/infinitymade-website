...
| 2026-09-11 | **`get_gmail_token`/`set_`/`clear_` anon-EXECUTE açığı (11.06–11.09.2026, 5 Konten): keine Meldung nach Art. 33/34; Dokumentation nach Art. 33 Abs. 5 + vorsorglicher Token-Widerruf** | Keine „Verletzung" i. S. d. Art. 4 Nr. 12 nachweisbar (Zugänglichkeit ohne belegten Zugriff; Art. 5 Abs. 2 Darlegungslast beachtet, EDSA Leitlinien 9/2022 v2.0 v. 28.03.2023: Kenntnis setzt „hinreichenden Grad an Gewissheit" einer Kompromittierung voraus). Zudem greift die Ausnahme Art. 33 Abs. 1 Hs. 2: **ein Google-Refresh-Token ist für einen confidential Client ohne `GOOGLE_CLIENT_SECRET` nicht einlösbar**; dieses Secret lag zu keiner Zeit im Repository (Leak v. 05.08.2026 betraf Fal-AI-Key, n8n-Key, Testpasswort). Scope ausschließlich `gmail.send` (`api-backend/server.js:468`) — kein Postfachzugriff, keine Patienten-/Gesundheitsdaten, **Art. 9 nicht einschlägig**, **§ 203 StGB nicht berührt**. Schreibrichtung (`set_`/`clear_`) betrifft nur Verfügbarkeit: fremde Tokens scheitern an `invalid_grant` (`server.js:542`) — eine solche Störung wäre vom Kunden bemerkt worden, keine Meldung ging ein. Betroffene = 5 Praxisinhaber (B2B), **Rolle: Verantwortlicher**, nicht Auftragsverarbeiter. Maßnahme: echter `oauth2.googleapis.com/revoke`-Aufruf **vor** Neuverbindung (bloßes Reconnect widerruft Alt-Tokens nicht) + Kundeninformation ohne „Datenpanne"-Wortlaut. Nebenbefund: Kenntnis lag seit `fortschritte/2026-08-29.md:183` vor, die dort selbst verlangte legal-de-Bewertung erfolgte erst am 11.09. — folgenlos, da keine Meldepflicht, künftig 72-h-Regel für Credential-/PHI-Funde. → `compliance/incidents/2026-09-11-gmail-token-rpc.md` | Kapalı — Umsetzung offen | Refresh-Test ohne Secret liefert `200` statt `401 invalid_client` · `GOOGLE_CLIENT_SECRET` taucht in der Git-Historie auf · Hinweis auf tatsächlichen Zugriff (Google-Metrics, Kundenmeldung) |
