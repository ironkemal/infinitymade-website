...
- [x] **2.1a TAMAM (11.09.2026)** — `onprem/docker-compose.yml` + `.env.template` + `NOTICE.md` + `volumes/` yazıldı ve **boş bir veritabanına karşı çalıştırıldı**: 7 konteyner sağlıklı, zincir kendi kendine koştu (`✓ 0000_baseline.sql`, 16,1 sn), 11 self-check sayacının 11'i tuttu, signup → trigger → profil oluştu, apikey'siz istek 401 aldı. Yığın **11 fremd konteynerden 6'ya** indi (studio · meta · imgproxy · supavisor · functions çıktı, her biri koda karşı doğrulandı). Compose'da **tek bir sürüm etiketi yok** — hepsi `.env`'den (O-45 çözüm (a)). Boşta ölçüm: **≈1,65 GB**, `api` dahil.
- [ ] 2.1b Caddy (TLS + statik yüzeyin servisi) · `pg_net`'siz kendi init dosyamız (O-49) · Kong ↔ Caddy kararı (O-48, konsey) · compose'un kutuya dağıtımı (O-45 (b)).
- [ ] 2.1c `install.sh`: donanım ön-kontrolü (2 vCPU/4GB/40GB + AB-lokasyon uyarısı), Docker kurulumu, `.env` üretimi (sırlar sunucuda üretilir — G2), compose up.
...
| 1 — Konsolidasyon | 🟡 **1.7 TAMAM** (şema zinciri) — 1.1–1.6 başlamadı | 2026-09-04 · 2026-09-10 | **1.7 = playbook'ta olmayan, sonradan açılan görev** (`onprem/SCHEMA-VERTEILUNG.md`). 04.09: runner yazıldı (`api-backend/db/migrate.js` + 16 test), `server.js`'e açılış öncesi bağlandı, hata olursa **bakım moduna** geçiyor (`process.exit` yok); `Dockerfile`'a `COPY db ./db`; tutarlılık kapısı (`tools/check-onprem.sh`, 7 sayaç) + yıkıcı-DDL kontrolü. 10.09: **`0000_baseline.sql` üretildi** (12.517 satır; `pg_dump --schema-only --no-owner --schema=public` + §2'nin dört eksik bölümü canlıya sorularak — 4 extension · `on_auth_user_created` · 5 storage kovası + 9 policy · realtime publication). 11 tablo bilinçli dışarıda (5 yabancı + 3 merkez + `scraper_data`→`b2b_contacts`→`email_logs` FK kümesi), 8 merkez fonksiyonu + Telegram trigger'ı baseline sonunda düşüyor (G1). **Ertelendi:** `profiles`'ın 10 Stripe kolonu → Faz 3.3 (bugün 21 yerde `plan_status` okunuyor, O-31) · postgis çıkarılması. Canlıda `praxura_migrations` defteri açıldı (RLS açık, policy yok, anon yetkisi geri alındı), `0000` = *uygulanmış* olarak kaydedildi — **baseline canlıda çalıştırılmadı**, SaaS zaten o şemada. `db/migrations-geplant/` → `archive/db-migrations-geplant-vor-baseline/` (üçünün içeriği de baseline'ın içinde, doğrulandı). Drift kapısının eksik yarısı yazıldı: döküm değişip zincire dosya girmezse commit reddediliyor. ✅ **Kanıt geldi (10.09 akşamı):** Docker kuruldu, yığın boş veritabanıyla ayağa kalktı, baseline `ON_ERROR_STOP=1` ile **hatasız yüklendi** (çıkış 0); 11 self-check sayacının 11'i canlıyla birebir; signup → trigger → profil oluştu; `no_overlapping_bookings` ayakta; veritabanından dışarı çıkan çağrı kalmadı. Test **üç gerçek hata** yakaladı (koşulsuz `CREATE SCHEMA public` — Temmuz PoC'sinin postgis'i kaybetmesinin de sebebi · `add_credits` çıkarılan `user_credits`'e öneksiz yazıyordu · `search_path` boş olduğu için storage policy'leri `profiles`'ı bulamıyordu), üçü de düzeltildi. ★ **Bağlayıcı bulgu:** kutunun `DATABASE_URL`'i **`supabase_admin`** olmalı — `postgres` `ALTER DEFAULT PRIVILEGES FOR ROLE supabase_admin` satırlarını çalıştıramıyor (V-6 "doğrulanmalı" notunun cevabı). ⚠️ Temmuz PoC'sinin `volumes/db/data` klasörü diske bağlı, `down -v` silmiyor — temiz kurulum testi için elle silinmeli. ⚠️ SaaS'ta `DATABASE_URL` hâlâ set değil → runner "übersprungen" diyor; set etmek ayrı ve bilinçli adım |
| 2 — Paket | 🟡 **2.1a + 2.8 TAMAM** — 2.0, 2.1b/c, 2.2–2.7, 2.9 açık | 2026-09-11 | **Kutunun compose'u artık var ve çalıştığı ölçüldü.** `onprem/docker-compose.yml` (+ `.env.template`, `NOTICE.md`, `volumes/`) yazıldı; **boş** bir veritabanına karşı ayağa kaldırıldı. Sonuç: 7/7 konteyner sağlıklı; migration zinciri **kendi kendine** koştu (`[migrate] ✓ 0000_baseline.sql (16119 ms)`) — zincirin kutuda ilk gerçek koşusu; 11 self-check sayacının 11'i canlıyla birebir (77 tablo = 76 + `praxura_migrations`, 293 index = 292 + pkey); signup → trigger → profil (auth 1 / profiles 1); apikey'siz PostgREST **401**; `praxura_migrations` PostgREST'ten **42501**; `/functions/v1` ve studio catch-all **404** (kaldırıldı). Yığın **11 → 6** fremd konteyner. Compose'da sürüm etiketi yok, hepsi `.env`'de (O-45 (a)). Boşta **≈1,65 GB** (`api` dahil; Kong tek başına 886 MB → O-48). **Test üç hata yakaladı:** (1) Google env'leri olmadan backend hiç açılmıyordu → düzeltildi (2.8, O-47); (2) `webhooks.sql`'i çıkarmak `roles.sql`'i kırıyor ve Storage'ı öldürüyor → geri alındı, O-49; (3) runner defter tablosunu RLS'siz yaratıyordu, kutu canlıdan farklı çıkıyordu → `migrate.js` düzeltildi. Yeni kapı: `onprem_image=7` + `check-onprem-volumes.sh` (upstream sapması) |
| 3 — Lisans | ⬜ başlamadı | | |
...
sayı tabandan artarsa commit reddediliyor. 10.09.2026'da yeniden ölçüldü — aradaki ~40
commit'te (§302, Kassenbuch, Verordnung, Mahnwesen) bulut zinciri **bir satır büyümedi**.
